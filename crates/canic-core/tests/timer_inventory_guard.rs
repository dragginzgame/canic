...
fn timer_provider_graph_and_manifest_consumers_are_closed() {
    ...
    assert_eq!(locked_package_versions(&lock, "icydb"), ["0.249.1"]);

    let workspace_manifest = read_source(&root, "Cargo.toml");
    assert!(workspace_manifest.contains("ic-timers = \"=0.7.0\""));
    assert!(workspace_manifest.contains("icydb = { version = \"0.249.1\""));
    assert!(workspace_manifest.contains("icydb-model = \"0.249.1\""));
