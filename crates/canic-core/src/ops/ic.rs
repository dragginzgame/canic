...
//!
//! These wrappers call into `crate::interface::*` (pure transport) and attach
//! ops-level concerns such as metrics recording.

use crate::{
    Error,
    cdk::mgmt::{
        self, CanisterInstallMode, CanisterStatusResult, DeleteCanisterArgs, InstallCodeArgs,
        UpdateSettingsArgs, WasmModule,
    },
    cdk::utils::wasm::get_wasm_hash,
    env::nns::NNS_REGISTRY_CANISTER,
    interface::{ic as ic_interface, prelude::*},
    log,
    log::Topic,
...
pub async fn create_canister(
    ...
    let pid = ic_interface::canister::create_canister(controllers, cycles).await?;
...
pub async fn canister_status(canister_pid: Principal) -> Result<CanisterStatusResult, Error> {
    let status = ic_interface::canister_status(canister_pid).await?;
...
pub fn canister_cycle_balance() -> Cycles {
    ic_interface::canister_cycle_balance()
}
...
pub async fn deposit_cycles(canister_pid: Principal, cycles: u128) -> Result<(), Error> {
    ic_interface::deposit_cycles(canister_pid, cycles).await?;
...
pub async fn uninstall_code(canister_pid: Principal) -> Result<(), Error> {
    ic_interface::uninstall_code(canister_pid).await?;
...
pub async fn update_settings(args: &UpdateSettingsArgs) -> Result<(), Error> {
    ic_interface::update_settings(args).await?;
