...
mod tests {
    ...
    fn resource_scope_matches_placement_capacity_authority() {
        ...
        let index_a = PlacementAllocationIdentity::index(p(1), "pool", "a", 1, &role, None);
        let index_b = PlacementAllocationIdentity::index(p(1), "pool", "b", 1, &role, None);
        assert_ne!(index_a.resource_key, index_b.resource_key);
    ...
    #[test]
    fn index_claims_and_shard_generations_advance_operations_not_capacity_scope() {
        let role = CanisterRole::new("worker");
        let index_first = PlacementAllocationIdentity::index(p(1), "pool", "key", 1, &role, None);
        let index_next = PlacementAllocationIdentity::index(p(1), "pool", "key", 2, &role, None);
        assert_ne!(index_first.operation_id, index_next.operation_id);
        assert_eq!(index_first.resource_key, index_next.resource_key);
