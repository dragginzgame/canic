...
/// The default domain separator for signatures generated by this canister.
/// Override if you need application-specific domains.
pub const DEFAULT_DOMAIN: &[u8] = b"canic-signature";
...
///
pub fn prepare(seed: &[u8], message: &[u8], domain: Option<&[u8]>) {
...
///
#[must_use]
pub fn get(seed: &[u8], message: &[u8], domain: Option<&[u8]>) -> Option<Vec<u8>> {
    let domain = domain.unwrap_or(DEFAULT_DOMAIN);
    let sig_inputs = CanisterSigInputs {
        domain,
        seed,
...
///
#[must_use]
pub fn sign(seed: &[u8], message: &[u8], domain: Option<&[u8]>) -> Option<Vec<u8>> {
    prepare(seed, message, domain);
    get(seed, message, domain)
}
