...
| `testing` | No | Host-only managed-App, managed Component-tree and standalone-local PocketIC qualification support; never compiled into Wasm roles and grants no runtime capability. |
...
`canic` dependency. `canic::testing` constructs exact managed init and runtime-
directory authority from the App's checked-in `canic.toml`, including
`ComponentChild` authority for Hub-created sharding, scaling and index
descendants. It drives fencing/activation and same-release upgrade, and re-
exports the bounded PocketIC call surface needed for application-specific
assertions. Downstreams do not need direct `canic-core` or `ic-testkit`
...
`canic.toml`. These are application canisters. Fleet Subnet Root is generated by the host from the App configuration; its
...
## Fleet Build Contracts

`canic-host` generates unpublished Root, Coordinator and Store entrypoint
