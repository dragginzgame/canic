...
"#,
    ...
    fn prepare_journey_infrastructure(
        ...
        // Separate paid setup from generation and the subsequent recovery proof.
        let mut phase = Span::start("prepare_infrastructure_creates");
        let paths = canic_host::fleet_ensure::ops::EnsurePaths::under(
            adapter_root,
            &desired.environment,
            &desired.fleet,
        ...
        phase = phase.next("prepare_infrastructure_installs");
        for action in actions
            .iter()
            .copied()
            .filter(|action| matches!(action, EnsureAction::Install { .. }))
        {
            let install = Span::start("prepare_infrastructure_install");
            install_prepared_infrastructure(pic, adapter_root, desired, plan, action, &state);
            install.finish();
        }
        phase = phase.next("prepare_infrastructure_controllers");
        for action in actions {
            match action {
    ...
    /// Install setup through PocketIC while the fresh and interrupted-install journeys
    /// retain production transport coverage. Init compilation and release binding stay shared.
    #[cfg(test)]
    fn install_prepared_infrastructure(
        pic: &PocketIc,
        adapter_root: &Path,
        desired: &DesiredFleet,
        plan: &FleetEnsurePlan,
        ...
        let EnsureAction::Install {
            name,
            canic_init: Some(init),
            init_arg: None,
            init_arg_sha256: None,
            init_candid: None,
            init_candid_sha256: None,
            mode: canic_host::fleet_ensure::model::InstallMode::Install,
            wasm,
            wasm_sha256,
            ..
        } = action
        else {
            panic!("prepared infrastructure requires a fresh typed Canic initializer");
        };
        assert!(
            state.principals.is_empty(),
            "setup has only freshly allocated identities"
        );
        assert_eq!(state.pending_principals.len(), desired.canisters.len());
        let bytes = std::fs::read(adapter_root.join(wasm)).unwrap();
        assert_eq!(
            canic_core::cdk::utils::hash::sha256_hex(&bytes),
            *wasm_sha256
        );
        let arguments = canic_host::fleet_ensure::ops::compile_arguments(
            &canic_host::fleet_ensure::ops::CanicInitRequest {
                ...
                init,
                operation_id: &plan.operation_id,
                principals: &state.pending_principals,
                root: adapter_root,
                wasm,
                wasm_sha256,
            },
        )
        .expect("compile exact production infrastructure arguments");
        pic.install_canister(
            Principal::from_text(&state.pending_principals[name]).unwrap(),
            bytes,
            arguments,
