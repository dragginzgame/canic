...
pub(super) fn write_report(
    ...
    out.push_str("- `root::canic_prepare_delegation_proof_batch` remains the main shared update hotspot in the retained audit lane, so further optimization work should stay focused on shared runtime/auth cost rather than demo provisioning flows.\n");
...
fn hotspot_hint(subject_label: &str) -> (&'static str, &'static str) {
    match subject_label {
        ...
        "canic_prepare_delegation_proof_batch" => (
            "Root delegated auth batch provisioning path",
            "[root endpoints](/home/adam/projects/canic/crates/canic/src/macros/endpoints/root.rs), [auth ops](/home/adam/projects/canic/crates/canic-core/src/ops/auth/delegation.rs)",
        ),
