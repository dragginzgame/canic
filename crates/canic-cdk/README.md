...
This crate currently declares MSRV `1.88.0`. The Canic repo itself builds with a
newer pinned internal toolchain, but downstream crates that compile
`canic-cdk` from source should only need Rust `1.88.0` or newer.
...
- `canic::cdk::structures` re-exports curated `ic_memory::stable_structures` types.
