...
impl DelegationRegistry {
    ...
    pub fn register_session(wallet_pid: Principal, args: RegisterSessionArgs) -> Result<(), Error> {
        ...
        if duration < MIN_EXPIRATION {
            return Err(
                OpsError::from(DelegationError::SessionTooShort(MIN_EXPIRATION.as_secs())).into(),
            );
        }
        if duration > MAX_EXPIRATION {
            return Err(
                OpsError::from(DelegationError::SessionTooLong(MAX_EXPIRATION.as_secs())).into(),
            );
        }
    ...
    pub fn resolve_wallet(session_pid: Principal) -> Result<Principal, Error> {
        ...
        if s.expires_at <= now_secs() {
            return Err(
                OpsError::from(DelegationError::SessionExpired(s.expires_at, now_secs())).into(),
            );
        }
    ...
    pub fn track(
        ...
        let mut session = Self::get(&session_pid)
            .ok_or_else(|| StateError::from(DelegationRegistryError::NotFound(session_pid)))?;
        let now = now_secs();

        if session.expires_at <= now {
            return Err(
                OpsError::from(DelegationError::SessionExpired(session.expires_at, now)).into(),
            );
        }
