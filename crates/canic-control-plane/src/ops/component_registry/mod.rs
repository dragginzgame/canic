...
use root_retirement::{
    root_final_inventory_hash, root_store_binding_finalization_hash,
    root_store_binding_finalization_record, root_store_deletion_hash, root_store_deletion_record,
    root_store_final_inventory_evidence, root_store_reclamation_hash,
    root_store_reclamation_record, validate_root_final_inventory_intent_record,
    validate_root_final_inventory_record, validate_root_store_deletion_authority,
};
...
mod tests {
    ...
    fn directory_refresh_is_exact_idempotent_and_conflict_closed() {
        ...
        assert!(
            ComponentRegistryOps::directory_synchronization_targets(&[component, component])
                .is_err()
        );
        let before = RootComponentRegistryStore::export();
        ...
        let plan =
            ComponentRegistryOps::prepare_directory_refresh(&target, fleet.clone(), None, 40)
        ...
        assert!(
            ComponentRegistryOps::prepare_directory_refresh(&target, fleet.clone(), None, 33)
                .is_err()
        );
        ...
        let committed = ComponentRegistryOps::commit_directory_refresh(
            &plan,
            root.limits.maximum_registry_bytes,
        )
        ...
        assert_eq!(
            ComponentRegistryOps::commit_directory_refresh(
                &plan,
                root.limits.maximum_registry_bytes,
            )
