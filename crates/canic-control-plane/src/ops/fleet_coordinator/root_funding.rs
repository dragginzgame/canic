...
impl FleetCoordinatorOps {
    ...
    pub(crate) fn prepare_root_funding(
        ...
        let fleet_window_start_secs =
            epoch_window_start(now_ns, coordinator_policy.budget.window_secs)?;
        let root_window_start_secs =
            epoch_window_start(now_ns, root.funding.root_funding.budget.window_secs)?;
        let fleet_reserved_cycles = reserved_fleet_cycles(&next, fleet_window_start_secs)?;
        let root_reserved_cycles = reserved_root_cycles(root_ledger, root_window_start_secs);
        let decision = decide_fleet_root_grant(&FleetRootGrantDecisionInput {
            funding_enabled: next.funding_enabled,
            registry_matches: request.expected_registry == registry_version,
            ...
            fleet_window: window_snapshot(
                next.fleet_window.as_ref(),
                fleet_window_start_secs,
                fleet_reserved_cycles,
            ),
            root_window: window_snapshot(
...
fn funding_acceptance_request(
    ...
    FleetRootFundingAcceptanceRequest {
        ...
        observed_balance: record.request.observed_balance,
        granted_cycles: record.request.requested_cycles,
...
) -> Option<FleetFundingWindowSnapshot> {
    ...
    match &result.response {
        FleetRootFundingResponse::Granted(receipt) => {
            let expected_request = FleetRootFundingAcceptanceRequest {
                ...
                observed_balance: result.request.observed_balance,
                granted_cycles: result.request.requested_cycles,
        ...
        FleetRootFundingResponse::NoGrant(receipt) => {
            if receipt.request != result.request
                || receipt.decided_at_ns != result.completed_at_ns
                || (receipt.reason != FleetRootFundingNoGrantReason::RootRejected
