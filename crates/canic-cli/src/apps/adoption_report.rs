//! Module: apps::adoption_report
//! Responsibility: build, render, and write `canic app adoption report` output.
//! Does not own: command dispatch, option parsing, app selection, or filesystem mutation outside report output.
...
pub(super) fn build_adoption_report_from_config_path(
    ...
    generated_at: &str,
) -> Result<AdoptionReportV1, AppCommandError> {
...
fn adoption_package_metadata_from_options(
    config_path: &Path,
    options: &AdoptionReportOptions,
) -> Result<Vec<AdoptionPackageMetadataV1>, AppCommandError> {
...
fn adoption_artifact_manifest_from_options(
    options: &AdoptionReportOptions,
) -> Result<Option<RoleArtifactManifestV1>, AppCommandError> {
...
fn adoption_inventory_from_options(
    options: &AdoptionReportOptions,
) -> Result<Option<DeploymentInventoryV1>, AppCommandError> {
...
fn read_deployment_check_inventory(path: &Path) -> Result<DeploymentInventoryV1, AppCommandError> {
...
fn read_json_file<T>(path: &Path) -> Result<T, AppCommandError>
where
    T: DeserializeOwned,
{
...
fn build_adoption_report_envelope(
    ...
    options: &AdoptionReportOptions,
    report: &AdoptionReportV1,
) -> Result<EvidenceEnvelopeV1, AppCommandError> {
...
fn adoption_report_input_fingerprints(
    config_root: &Path,
    options: &AdoptionReportOptions,
) -> Result<Vec<InputFingerprintV1>, AppCommandError> {
...
const fn adoption_package_state_label(state: AdoptionPackageStateV1) -> &'static str {
    match state {
        ...
        AdoptionPackageStateV1::MissingApp => "missing-app",
