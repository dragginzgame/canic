...
pub(super) struct NodeListOptions {
...
///
#[derive(Clone, Debug, Eq, PartialEq)]
pub(super) struct NodeInfoOptions {
...
///
#[derive(Clone, Debug, Eq, PartialEq)]
pub(super) struct NodeRefreshOptions {
    pub(super) network: String,
    pub(super) format: OutputFormat,
    pub(super) source_endpoint: String,
    ...
    I: IntoIterator<Item = OsString>,
{
    ...
    if print_help_or_version(&args, node_usage, version_text()) {
        return Ok(());
    }
    let (command, args) = parse_required_subcommand(node_command(), args)
        .map_err(|_| NnsCommandError::Usage(node_usage()))?;

    match command.as_str() {
        "list" => run_node_list(args),
        "info" => run_node_info(args),
        "refresh" => run_node_refresh(args),
        _ => unreachable!("nns node dispatch command only defines known commands"),
    }
}

fn run_node_list<I>(args: I) -> Result<(), NnsCommandError>
where
    I: IntoIterator<Item = OsString>,
{
    let args = args.into_iter().collect::<Vec<_>>();
    ...
    let options = NodeListOptions::parse(args)?;
...
fn run_node_info<I>(args: I) -> Result<(), NnsCommandError>
where
    I: IntoIterator<Item = OsString>,
{
    let args = args.into_iter().collect::<Vec<_>>();
    ...
    let options = NodeInfoOptions::parse(args)?;
    let icp_root = icp_root().map_err(|err| NnsCommandError::Usage(err.to_string()))?;
...
fn run_node_refresh<I>(args: I) -> Result<(), NnsCommandError>
where
    I: IntoIterator<Item = OsString>,
{
    let args = args.into_iter().collect::<Vec<_>>();
    ...
    let options = NodeRefreshOptions::parse(args)?;
        ...
        I: IntoIterator<Item = OsString>,
    {
        ...
        Ok(Self {
            ...
            source_endpoint: string_option(&matches, "source-endpoint")
                .unwrap_or_else(|| DEFAULT_NNS_NODE_SOURCE_ENDPOINT.to_string()),
...
impl NodeInfoOptions {
    ...
    where
        I: IntoIterator<Item = OsString>,
    {
        let matches = parse_matches(node_info_command(), args)
            .map_err(|_| NnsCommandError::Usage(node_info_usage()))?;
        Ok(Self {
...
impl NodeRefreshOptions {
    pub(super) fn parse<I>(args: I) -> Result<Self, NnsCommandError>
    where
        I: IntoIterator<Item = OsString>,
    {
        let matches = parse_matches(node_refresh_command(), args)
            .map_err(|_| NnsCommandError::Usage(node_refresh_usage()))?;
        Ok(Self {
            network: string_option(&matches, "network")
                .unwrap_or_else(|| MAINNET_NETWORK.to_string()),
            format: typed_option(&matches, "format").unwrap_or(OutputFormat::Text),
            ...
            lock_stale_after_seconds: typed_option(&matches, "lock-stale-after")
                .unwrap_or(DEFAULT_NODE_REFRESH_LOCK_STALE_SECONDS),
...
pub(super) fn node_command() -> ClapCommand {
    ClapCommand::new("node")
        .bin_name("canic nns node")
        .about("Inspect NNS node metadata")
        .disable_help_flag(true)
        .subcommand(passthrough_subcommand(
            ClapCommand::new("list").about("List cached mainnet NNS nodes"),
        ))
        .subcommand(passthrough_subcommand(
            ClapCommand::new("info").about("Show one cached mainnet NNS node"),
        ))
        .subcommand(passthrough_subcommand(
            ClapCommand::new("refresh").about("Force-refresh and cache NNS node metadata"),
        ))
}

fn node_list_command() -> ClapCommand {
    ClapCommand::new("list")
        .bin_name("canic nns node list")
        .about("List cached mainnet NNS nodes")
        .disable_help_flag(true)
        .arg(
            value_arg("format")
                .long("format")
        ...
        .arg(
            value_arg("source-endpoint")
                ...
                .value_name("url")
                .help("IC API endpoint used if the node cache is missing"),
        )
        .arg(
            flag_arg("verbose")
                .long("verbose")
                .help("Show full node principals and registry metadata in text output"),
        )
        .arg(internal_network_arg())
        .after_help(NODE_LIST_HELP_AFTER)
}

fn node_info_command() -> ClapCommand {
    ClapCommand::new("info")
        .bin_name("canic nns node info")
        .about("Show one cached mainnet NNS node")
        .disable_help_flag(true)
        .arg(
            value_arg("input")
                .value_name("node|node-prefix")
                .required(true)
                .help("Node principal or unique node principal prefix"),
        )
        .arg(
            value_arg("format")
                .long("format")
        ...
        .arg(internal_network_arg())
        .after_help(NODE_INFO_HELP_AFTER)
}

fn node_refresh_command() -> ClapCommand {
    ClapCommand::new("refresh")
        .bin_name("canic nns node refresh")
        .about("Force-refresh and cache NNS node metadata")
        .disable_help_flag(true)
        .arg(
            value_arg("format")
                .long("format")
        ...
        .arg(
            flag_arg("dry-run")
                .long("dry-run")
                .help("Fetch and validate without replacing the cached node report"),
        )
        .arg(
            value_arg("output")
                .long("output")
                .value_name("path")
                .help("Also write the fetched node JSON to this path"),
        )
        .arg(internal_network_arg())
        .after_help(NODE_REFRESH_HELP_AFTER)
}

pub(super) fn node_usage() -> String {
    render_help(node_command())
}
...
pub(super) fn node_info_usage() -> String {
    render_help(node_info_command())
}

pub(super) fn node_refresh_usage() -> String {
    render_help(node_refresh_command())
}
