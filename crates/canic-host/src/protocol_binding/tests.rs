...
fn immutable_infrastructure_binding_reproduces_full_profile() {
    ...
    let artifact_relative_path = format!(
        ".canic/release-builds/{release_build_id}/artifacts/fleet_coordinator/fleet_coordinator.wasm"
    );
    let did = root.join(&artifact_relative_path).with_extension("did");
    fs::create_dir_all(did.parent().expect("DID parent")).expect("create DID parent");
    fs::write(&did, candid).expect("write DID");
    let artifact = crate::release_set::CanicInfrastructureArtifactEntry {
        ...
        release_build_id,
        wasm_relative_path: artifact_relative_path,
        wasm_size_bytes: 1,
...
fn fleet_subnet_root_artifact_selects_the_root_protocol_role() {
    ...
    let artifact_relative_path =
        format!(".canic/release-builds/{release_build_id}/artifacts/root/root.wasm");
    let did = root.join(&artifact_relative_path).with_extension("did");
    fs::create_dir_all(did.parent().expect("DID parent")).expect("create DID parent");
    fs::write(&did, candid).expect("write DID");
    let artifact = crate::release_set::CanicInfrastructureArtifactEntry {
...
fn infrastructure_binding_rejects_a_compatibility_sidecar_without_the_exact_release_sidecar() {
    ...
    assert!(matches!(
        resolve_infrastructure_protocol_binding(&root, "local", &artifact),
        Err(ProtocolBindingError::MissingCandid { .. })
    ));
    fs::remove_dir_all(root).expect("remove temp root");
}
