...
fn renders_argument_update_call_with_local_candid() {
    ...
    assert_eq!(
        ...
        "icp canister call root canic_root_command (variant { RefillCycles = record { amount_e8s = 100000000 : nat64 } }) --candid .icp/local/canisters/root/root.did --json -e local"
    );
}
