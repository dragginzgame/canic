...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if confirmation == "--confirm-resume-store-deletion" {
        let store_binding_finalization: FleetSubnetRootStoreBindingFinalizationResponse = query(
            ...
            &FleetSubnetRootStoreBindingFinalizationStatusRequest { operation_id },
        )?;
        ...
        println!(
            "{}",
            serde_json::to_string_pretty(&StoreDeletionResumeReceipt {
                operation_id_hex,
                fleet_subnet_root,
    ...
    let registry: FleetRegistry = query(&icp, coordinator, protocol::CANIC_FLEET_REGISTRY, &())?;
    ...
    let _: FleetSubnetRootRegistryMirrorActivationResponse = call(
        ...
        &FleetSubnetRootRegistryMirrorActivationRequest {
            previous_registry: active_registry,
            expected_registry: published.version.clone(),
            expected_directory: directory,
            store_bootstrap: RootStoreBootstrapRequest {
                manifest_payload_size_bytes: store_bootstrap_manifest_bytes,
            },
        },
    )?;
