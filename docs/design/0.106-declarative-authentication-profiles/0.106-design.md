# Canic 0.106 Design: Declarative Authentication Profiles
...
- Release boundary: reinstall only. 0.106 consumes no prior-release token,
  certificate, issuer-policy, session or verifier state.
- Implementation approval: none.
- Sequence: this is the 0.106 design line. It follows the
  ...
  the
  [0.104 direct transport groundwork](../0.104-cross-subnet-data-transport-groundwork/0.104-design.md)
  and the proposed
  [0.105 Coordinator Worker concept](../0.105-coordinator-workers/0.105-design.md).
...
0.106 proposes one declarative **authentication profile** as the application
...
0.106 will make authentication profiles the canonical source of login-time
...
Before this mode is implementation-approved, 0.106 must freeze:
...
0.106 does not claim to remove that initial token-signature operation. Its
...
policy. 0.106 must not create another tokenless session record, decision
...
0.106 owns declarative profile compilation, issuance, verifier projections and
client acquisition.
...
0.106 is ready for implementation only when:
