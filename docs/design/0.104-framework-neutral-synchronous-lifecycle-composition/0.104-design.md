...
- Sequence: this line follows the
  ...
  and precedes the renumbered
  [0.105 Cross-Subnet data transport groundwork](../0.105-cross-subnet-data-transport-groundwork/0.105-design.md).
...
0.104 is not a timer-provider design. It neither replaces Canic timers nor
adopts another framework's provider. A participant may use its independently
qualified timer implementation to reconstruct its own watchdog.

Canic's timer inventory and metrics remain Canic authority. Another framework
owns its timer inventory until a separately designed shared timer substrate is
adopted by both. Lifecycle composition must not imply unified timer naming,
cancellation, metrics or performance accounting.
...
- a shared timer provider, watchdog API or unified timer metrics;
