# Canic 0.107 Design: Standalone Blob Service Extraction
...
- Release boundary: reinstall only. Canic 0.107 does not migrate, import,
  adopt or decode blob state written by Canic 0.69 through 0.106.
...
- Version ownership: `0.107` is the Canic integration/removal release, not the
...
- Repository boundary: the implementation belongs in a standalone
  project. Canic 0.107 owns the Canic hard cut and the ordinary Component
...
0.107 removes blob-storage product semantics from Canic and establishes a
...
generic capability and 0.107 pauses for a separate backend-neutral Canic
...
- require Canic 0.107 to wait for Toko/IcyDB production deployment;
...
0.107 keeps the identities separate:
...
0.101 schema. 0.107 must not add a blob-specific configuration table.
...
hard cut. Canic 0.107 pins the exact standalone Wasm as an ordinary application
...
does not block Canic 0.107 closeout once the qualified standalone release and
...
0.107 removes the current blob subsystem rather than wrapping it.
...
IDs 55 through 58 become unallocated `canic-core` reserve in 0.107. They are
...
Canic 0.107 is a fresh installation. Existing Canic-embedded blob state is not
...
mixed-version operation are outside 0.107. A future service release must
...
production adoption remains downstream work and is not a Canic 0.107 release
dependency.
...
- optional Canic backup archival, mandatory client-side encryption,
  ...
  [0.109 encrypted canister-snapshot archive design](../0.109-optional-encrypted-canister-snapshot-archives/0.109-design.md);
