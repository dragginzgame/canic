# Canic 0.105 Design: Cross-Subnet Data Transport Groundwork
...
- Release boundary: reinstall only. 0.105 installs one current call surface
...
- Sequence: this is the 0.105 design line. It follows:
  ...
  - the independent [0.103 framework-neutral local application authorization design](../0.103-framework-neutral-local-application-authorization/0.103-design.md); and
  - the independent [0.104 synchronous lifecycle-composition design](../0.104-framework-neutral-synchronous-lifecycle-composition/0.104-design.md).
...
- Product-path posture: 0.105 is not required for client-directed application
...
- Independence posture: 0.105 neither requires nor extends 0.103 application
...
- Lifecycle posture: 0.105 neither requires nor extends the 0.104 synchronous
  participant seam. Its call wrapper remains ordinary application/runtime work
  after lifecycle restoration.

## Summary

0.105 prepares Canic for a later data path in which one read-only replica or
...
transport, topology, consistency and recovery into one layer. 0.105 instead
...
0.105 must not hard-code current cycle prices. Platform prices and Subnet
...
0.105 qualifies and wraps those current primitives; it does not invent a
...
None of those behaviors is implemented in 0.105.
...
operator diagnostics, installation qualification and reconciliation. 0.105
...
after the route is derived. In particular, 0.105 must not assume that a Cloud
...
0.105 adds an explicit bounded timeout to the Canic wrapper rather than relying
...
database timeout is selected in 0.105.
...
0.105 uses this provisional value only for its Fleet data-call qualification
fixture:
...
No 0.105 endpoint splits, chunks, compresses or rejoins data. The qualification
...
0.105 therefore names the value `required_cycles_reserve`, not `cost`,
...
0.105 requires typed distinctions for:
...
diagnostic codes. If 0.105 is implemented first, they remain typed producers
...
0.105 does not add encryption. Inter-Canister messages use the IC's replicated
...
request is not commitment. 0.105 does not weaken the Fleet-scoped identity or
...
Registry revision and root mirror activation. 0.105 may reuse its:
...
0.105 does not add Registry deltas or reinterpret
...
Replicas, or one or more PoolMembers. 0.105 resolves an exact selected
...
application data or read eligibility. 0.105 does not:
...
0.102 owns the maintained compact diagnostic representation. 0.105 adds only
typed failure producers and does not introduce transport error prose or a
second error envelope.

## Explicit Non-Goals

0.105 does not define or implement:
...
0.105 also does not change client-directed application routing or local
...
5. Confirm no database, replica or application-data behavior entered 0.105.
...
0.105 groundwork is complete only when:
...
After 0.105, a separate design may define one read-only replica or read-model
protocol:
