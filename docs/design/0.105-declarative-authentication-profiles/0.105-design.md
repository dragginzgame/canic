# Canic 0.105 Design: Declarative Authentication Profiles
...
- Release boundary: reinstall only. 0.105 consumes no prior-release token,
  certificate, issuer-policy, session or verifier state.
- Implementation approval: none.
- Sequence: this is the 0.105 design line. It follows the
  ...
  the
  [0.103 direct transport groundwork](../0.103-cross-subnet-data-transport-groundwork/0.103-design.md)
  and the proposed
  [0.104 Coordinator Worker concept](../0.104-coordinator-workers/0.104-design.md).
...
0.105 proposes one declarative **authentication profile** as the application
...
0.105 will make authentication profiles the canonical source of login-time
...
| delegated session | optional bounded local mapping from transport caller to authenticated subject |
...
If a valid Canic delegated session exists, the token remains bound to the
resolved authenticated subject under the existing caller-lane separation.
Topology predicates continue to consume the raw transport caller.
...
Before this mode is implementation-approved, 0.105 must freeze:
...
0.105 does not claim to remove that initial token-signature operation. Its
...
The Candid contract continues carrying the token as argument zero so the
proof is visible and independently auditable. Client helpers may inject that
argument but must not create a tokenless wire path.
...
0.105 is ready for implementation only when:
