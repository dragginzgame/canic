# Canic 0.104 Design: Cross-Subnet Data Transport Groundwork
...
- Release boundary: reinstall only. 0.104 installs one current call surface
...
- Sequence: this is the 0.104 design line. It follows:
  ...
  - [0.102 compact diagnostic codes](../0.102-compact-diagnostic-codes/0.102-design.md); and
  - the independent [0.103 framework-neutral local application authorization design](../0.103-framework-neutral-local-application-authorization/0.103-design.md).
...
- Product-path posture: 0.104 is not required for client-directed application
...
- Independence posture: 0.104 neither requires nor extends 0.103 application
...
0.104 prepares Canic for a later data path in which one read-only replica or
...
transport, topology, consistency and recovery into one layer. 0.104 instead
...
0.104 must not hard-code current cycle prices. Platform prices and Subnet
...
0.104 qualifies and wraps those current primitives; it does not invent a
...
None of those behaviors is implemented in 0.104.
...
operator diagnostics, installation qualification and reconciliation. 0.104
...
after the route is derived. In particular, 0.104 must not assume that a Cloud
...
0.104 adds an explicit bounded timeout to the Canic wrapper rather than relying
...
database timeout is selected in 0.104.
...
0.104 uses this provisional value only for its Fleet data-call qualification
fixture:
...
No 0.104 endpoint splits, chunks, compresses or rejoins data. The qualification
...
0.104 therefore names the value `required_cycles_reserve`, not `cost`,
...
0.104 requires typed distinctions for:
...
diagnostic codes. If 0.104 is implemented first, they remain typed producers
...
0.104 does not add encryption. Inter-Canister messages use the IC's replicated
...
request is not commitment. 0.104 does not weaken the Fleet-scoped identity or
...
Registry revision and root mirror activation. 0.104 may reuse its:
...
0.104 does not add Registry deltas or reinterpret
...
Replicas, or one or more PoolMembers. 0.104 resolves an exact selected
...
application data or read eligibility. 0.104 does not:
...
0.102 owns the maintained compact diagnostic representation. 0.104 adds only
typed failure producers and does not introduce transport error prose or a
second error envelope.

## Explicit Non-Goals

0.104 does not define or implement:
...
0.104 also does not change client-directed application routing or local
...
5. Confirm no database, replica or application-data behavior entered 0.104.
...
0.104 groundwork is complete only when:
...
After 0.104, a separate design may define one read-only replica or read-model
protocol:
