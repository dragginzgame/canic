...
- State: accepted and scheduled as application-safety/estate step 8, but its
  scope must be retriaged after the binding 0.109 B8 release/adoption-support
  gate and B9 complexity-remediation gate.
...
- Predecessors: completed 0.103 and 0.104, accepted current 0.106
  ...
  Coordinator-backed root funding, completed 0.109 Fleet-wide ingress
  admission, completed B8 release/adoption support and an accepted passing B9
  superseding verdict for the
  [0.109 post-implementation complexity audit](../../audits/release-lines/0.109-post-implementation-complexity-audit.md).
- Implementation approval: none. B1 additionally requires completed 0.109 B8
  release/adoption support, B9 simplification, a passing immutable complexity
  verdict, explicit
  retain/defer/remove review of every planned 0.110 batch and a fresh
  maintainer promotion. Publication, functional completion or generic
  continuation cannot substitute for those gates.
...
| --- | --- | --- | --- | --- |
| B1 | Indexed estate authority | Hard-cut policy, counters, indexes, pagination, transitions and scan removal | State/property, first-excess and corrupted-index tests | Blocked on 0.109 B8 release/adoption support, B9 complexity remediation, complete scope retriage and fresh promotion |
...
No 0.110 mutation is authorized. Finish 0.109 B8 release/adoption support,
finish every B9 complexity finding, obtain an accepted passing immutable
verdict, then reclassify every
0.110 batch as retained, deferred or removed against downstream need and the
accepted complexity/validation budget. Only a later explicit maintainer
decision may promote the resulting B1.
