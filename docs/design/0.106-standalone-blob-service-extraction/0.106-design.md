...
- Status: accepted for M0 investigation; full implementation remains proposed.
...
- require Canic 0.106 to wait for Toko/IcyDB production deployment;
...
hard cut. Canic 0.106 pins the exact standalone Wasm as an ordinary application
...
does not block Canic 0.106 closeout once the qualified standalone release and
...
production adoption remains downstream work and is not a Canic 0.106 release
dependency.
...
- optional cost-gated Canic backup archival, mandatory client-side encryption,
  recovery-capsule discovery and native repository automation are owned by the
  tentative
  [0.108 encrypted canister-snapshot archive design](../0.108-optional-encrypted-canister-snapshot-archives/0.108-design.md);
