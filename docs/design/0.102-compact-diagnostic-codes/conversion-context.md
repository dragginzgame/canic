...
| --- | --- | --- |
| `PublicationWorkflowError` | Maps variants to four Store leaves plus broad conflict, invariant and unavailable | Match every variant to an approved Store/publication leaf; retain typed metrics decisions |
| `TemplateManifestOpsError` | Maps selected variants to Store leaves and flattens the remainder to `Ops` prose | Split every remainder by exact invariant or rejection meaning |
...
| `RpcWorkflowError` | Makes disabled funding, operation ID, funding policy and in-progress public; flattens the rest | Preserve distinct funding limit/cooldown decisions and map every remaining workflow variant |
