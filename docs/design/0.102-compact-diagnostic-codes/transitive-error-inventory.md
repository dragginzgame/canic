...
`AuthScopeError` and `AuthExpiryError`. Scope and expiry are already structurally
typed. Three input buckets still lose identity:
...
[direct-constructor-frontier.md](direct-constructor-frontier.md) remains open.
...
The mapping is provisional until direct-constructor reconciliation and
maintainer allocation review; a dependency version/checksum change reopens it.
...
- `IcpRefillWorkflowError`: three direct workflow failures plus seven typed
  `IcpRefillPolicyViolation` reasons beneath `PolicyDenied`; and
...
one code per displayed variant automatically.
...
## Next Allocation Passes

The provisional leaf table is being completed in bounded semantic families, in
this order:

1. configuration and compiled topology, now provisionally recorded in
   [configuration-leaves.md](configuration-leaves.md) and awaiting exact
   path-specific producer proof;
2. authentication, policy and authority fences, provisionally recorded in
   [auth-policy-leaves.md](auth-policy-leaves.md) except for the nested typed
   producers currently erased by `AuthValidationError::Auth(String)`;
3. IC infrastructure plus the `ic-memory` adapter;
4. Fleet Registry, provisioning, runtime and stable-state operations; and
5. standalone workflow owners plus the direct public-construction ledger.

Each family must record candidate label, exact producer variants, class,
origin, public projection, action/retry behavior and masked observability owner.
Numeric assignment remains the final B1 action after the families reconcile
with the 34 live Component policy variants.
