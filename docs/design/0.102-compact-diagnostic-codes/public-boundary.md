...
| `canic-core/api/blob_storage` | 11 | Current blob lifecycle and Cashier mapping pending the 0.107 hard cut |
...
| `InternalRpcMalformed` | Cashier response mapping in the current blob subsystem | Cover while current; do not preserve after the independent 0.107 hard cut removes its producer |
...
| `NotFound` | Current blob object not live | Cover the current producer; 0.107 determines its later retirement |
