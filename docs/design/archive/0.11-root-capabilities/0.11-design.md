...
Replay identity uses two bindings:

1. replay slot key (storage identity)
2. replay conflict digest (capability envelope binding)

Shared replay alignment:

- Root replay is a profile of the shared replay model introduced in 0.13.
- In that model, root requests map to `target_canister = root_canister_id` and `service = Root`.

#### 8.1.1 Replay Slot Key

A replay slot MUST be uniquely identified by:
...
Unified shared form (0.13-compatible profile):
...
- 0.11 metadata does not carry `nonce`; shared implementations **MAY** map missing nonce to `ZERO_16`.
- New 0.13-style callers **SHOULD** provide explicit nonce.
...
Including `capability_discriminant` avoids slot-level collisions when a caller
reuses the same `request_id` across different capability variants. Implementations
that do not include `capability_discriminant` remain safe because `ReplayDigest`
conflict checks are still mandatory.

#### 8.1.2 Replay Conflict Digest

Each replay slot MUST bind a deterministic payload digest:

```text
ReplayDigest = hash(canonical_capability_envelope_bytes)
```

Where canonical capability envelope bytes are:

- deterministic canonical serialization
- include exact capability variant discriminant
- include canonical request payload fields
- exclude `RootRequestMetadata` when metadata is transported out-of-payload

Canonical serialization requirements:

- Canonical capability serialization **MUST** use Candid canonical encoding.
...
- Cross-caller replay
- Cross-subnet replay
- Payload mutation under the same slot key

`request_id` requirements:

- `request_id` **MUST** be collision-resistant.
- `request_id` **MUST** be 32 bytes generated from a cryptographically secure randomness source.
- `request_id` **MUST NOT** be derived solely from request payload hashing.
...
- For each replay slot key (per Section 8.1.1 baseline or hardened form), Root **MUST** bind and store the first accepted `ReplayDigest`.
- Reuse of that slot with a different `ReplayDigest` **MUST** be rejected as conflicting replay.
- Reuse of that slot with the same `ReplayDigest` **MUST** return the cached response.
...
- Existing slot checks (expiry/conflict/cache-hit) **MUST** execute before any opportunistic purge sweep.
...
## 16. Implementation Deltas (0.11.x)

These deltas capture the implemented behavior relative to earlier draft wording.

### 16.1 Replay Key Composition

- Replay storage identity in 0.11.x implementation is `ReplaySlotKey = H(caller || subnet || request_id)`.
- 0.13+ shared replay profile maps root to `target=root`, `service=Root`, and includes `nonce`.
- Capability/variant binding is enforced through `ReplayDigest = H(canonical capability envelope bytes)`.
- Canonical capability envelope hashing includes the enum discriminant and excludes replay metadata.

### 16.2 Replay Resolution Semantics

For an existing replay slot:

- same `ReplayDigest` -> return cached response
- different `ReplayDigest` -> reject as replay conflict
- expired entry -> reject as expired replay (no resurrection)

### 16.3 Purge Ordering

- Existing replay slot evaluation occurs before opportunistic expired-entry purge.
- This preserves explicit expired/conflict outcomes instead of silently treating old slots as new requests.
