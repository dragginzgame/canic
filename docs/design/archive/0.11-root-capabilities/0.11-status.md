...
- [x] Ensure replay slot key is scoped by shared replay identity `(caller, target, service, request_id, nonce)` with legacy 0.11 compatibility.
...
- [x] Root replay threshold-key behavior is automatic (no env gating toggle).
- [x] CI replay coverage no longer depends on threshold-key environment flags.
