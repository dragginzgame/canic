...
- State: 0.108.0 is published. The first human closeout audit rejected the open
  ...
  Ledger/CMC plus real single- and multi-Root journeys. Targeted correction
  validation and evidence reconciliation are complete; a new human verdict is
  still required before the maintainer-owned validation/version/publication
  workflow.
- Runtime impact: B3 adds exact registered-Root admission, the Coordinator
  ...
  fences. B8 qualifies both real funding paths and reconciles active docs.
- Maintainer amendment: on 2026-08-22 the funding design replaced the earlier
  ...
  professional multi-Subnet 250T/1000T profile, 30-day cooldowns,
  one-grant-per-90-day windows and finite non-renewing automatic caps. It also
  hard-cuts the Fiduciary-backed `recommended` selector: each Fiduciary
  placement now needs an exact protected-input acknowledgement and retained
...
- Surface posture: 0.108.0 remains policy-only. The open draft adds Coordinator
  ...
  `canic cycles funding`. Direct top-up and conversion resolve only exact
  authenticated installed Coordinator/current-Root authority.
...
| B5 | Sparse-policy correction and Root timer integration | Topology defaults, finite non-renewing caps, explicit Fiduciary placement, low-balance request ownership, nonterminal resumption and unchanged descendant funding | Host/config/hash, timer/policy/restart, placement-warning, generated-Candid and Root/Coordinator artifact checks | Complete 2026-08-22; open 0.108.1 draft |
...
| B8 | Qualification and closeout readiness | Real value-transfer and fallback journeys, measured guidance, sediment audit and closeout handoff | Targeted repository gates and PocketIC evidence | Initial closeout rejected; corrections and targeted validation complete; re-audit pending |
...
Establish the immutable corrected candidate revision, then run a fresh
human-owned 0.108 closeout audit against the complete open 0.108.1 batch. Do
not begin 0.109, run remote qualification effects, version, tag or publish
until that verdict and the maintainer-owned release workflow authorize those
actions. The published 0.108.0 checkpoint and held 0.106 B2 work remain
independent.
