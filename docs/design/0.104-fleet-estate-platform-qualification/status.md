...
- Predecessor reconciliation: repository-only B1 may continue while 0.103 is
  pending, but B2 and the final current baseline require accepted 0.103 timer,
  recovery and stable-state ownership.
- Successors: 0.105 consumes the accepted B1 root-ownership/current-cost
  boundary and is not gated on B2. Accepted B2 evidence is mandatory for
  mutating 0.106 Fleet-estate work.
...
| B2 | Dated platform qualification and accepted 0.106 handoff | qualification harness and audit report | protocol-bound disposable measurements, separately approved minimal mainnet confirmation, qualified cost/balance recommendations and complete reconciliation | run-specific checks plus maintainer evidence review | Execution held: blocked on accepted B1 and explicit external authorization |

The two-batch exception is intentional: 0.104 is an evidence-only predecessor
...
5. strict empty-topology host plan and PocketIC activation proof, or an exact
   accepted blocker assigned to 0.106;
...
If mainnet authorization is not granted, B2 and 0.104 are blocked, not failed,
and cannot be marked complete.

## Critical-Path Position

0.104 is reserve-Fleet critical-path step 2:

1. 0.103 completes the timer-consumer and async-job recovery hard cut;
2. 0.104 qualifies the platform and current pool boundary;
3. 0.105 closes replay-safe Coordinator-backed root operating funding; and
...
Do not run a remote or IC-mainnet experiment or begin 0.106 production work.
