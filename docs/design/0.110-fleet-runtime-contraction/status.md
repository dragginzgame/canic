...
- Review verdict: the frozen replica-validator gate and whole generated-surface
  amendment are binding. Predecessor closeout and explicit promotion remain
  mandatory.
- Outcome: create durable absolute Wasm code-section and replica-validator
  function headroom through storage, codec and whole generated-surface
...
- B1 freezes dated code-section, total-module and declared-function limits.
  Completion requires at least 350 KiB absolute code-section headroom and at
  least 5% of the declared-function limit free under a frozen
  replica-equivalent validator; 500 KiB code headroom is preferred. Defined
...
- Every B2-B4 batch ends with canonical roles, the endpoint-heavy fixture,
  fresh marginal attribution and an explicit stop/continue/retarget decision.
- B3 records/codecs and B4 endpoint/recovery cuts proceed only where the latest
  residual evidence justifies them.
...
| B4 | Endpoint, recovery and role-capability pruning | Complete generated-surface inventory, exact Candid/provider reachability, optimized body/function evidence, direct dispatch, gated hard deletion of the temporary pool Ledger recovery family, role pruning and full remeasurement | Blocked on B3 decision |
| B5 | Final downstream-shaped qualification and closeout | Absolute byte/function reserves, per-role generated-surface absence, total-module limit, instructions, determinism, reinstall guard and immutable audit | Blocked on final B2-B4 decision |
