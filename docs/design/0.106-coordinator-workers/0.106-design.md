# Canic 0.106 Design: Coordinator Workers
...
0.106 may let the Coordinator create Workers as demand grows, but creation
...
Before implementation, 0.106 must freeze:
...
0.106 is ready for implementation only when one exact Worker kind completes
