...
- Sequence: this is the 0.103 design line. It follows:
  ...
  - [0.101 composable Component deployment and Fleet service publication](../0.101-fleet-authoritative-service-provisioning-and-publication/0.101-design.md); and
...
0.103 prepares Canic for a later data path in which one read-only replica or
...
defines Canic `SubnetKind::{CloudEngine, Public}`, the host-side
registry-version-bound provider observation and the eventual protected
`SubnetPlacement`. `SubnetKind` is independent of route:

~~~text
same Subnet ID + either kind       → SameSubnet
different Subnet IDs + either kind → CrossSubnet
~~~
...
after the route is derived. In particular, 0.103 must not assume that a Cloud
Engine supports every public-Subnet combination of cross-Subnet attached
cycles, bounded wait and unbounded wait. The exact current platform behavior
must be qualified before implementation; unsupported effects reject before
dispatch or use a separately designed public-Subnet proxy with its own
authority, funding and receipt model.
...
Replicas, or one or more PoolMembers. 0.103 resolves an exact selected
...
application data or read eligibility. 0.103 does not:
