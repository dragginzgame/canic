...
they are the read-only consumer and canonical data authority. A Tree Root
distributes protected topology and owns local lifecycle; it is not an implicit
data proxy. Any future relay must be an explicitly declared Tree role with its
own protocol.
...
- a Tree-bound Canister uses its own `TreeId` and the corresponding locally
  verified Fleet Directory placement;
- a Tree Root additionally has its Fleet Registry Mirror and protected
  `TreeBinding`;
...
- a Fleet-authoritative service resolves its exact
  `FleetServiceBinding { role, tree, canister_id }`, then resolves
...
- the Fleet Coordinator resolves from
  ...
  calls only;
- the Tree Root-owned Tree Registry for a locally managed Canister; or
...
A separately supplied public Fleet or Tree Directory response is only an
untrusted hint.
...
- canonical `FleetServiceBinding`, including role, owning `TreeId`
  and service principal;
- the target Tree's exact Tree Root and Tree Spec binding where relevant;
- allowed caller Tree, caller role and caller Canister;
...
`FleetServiceBinding { role, tree, canister_id }`. 0.103 resolves the
...
7. How are idempotency keys, expected data revision and committed result
   receipts encoded?
8. What exact caller Tree, caller role, caller principal and canonical
