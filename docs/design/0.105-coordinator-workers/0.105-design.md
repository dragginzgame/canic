# Canic 0.105 Design: Coordinator Workers
...
0.105 may let the Coordinator create Workers as demand grows, but creation
...
Before implementation, 0.105 must freeze:
...
0.105 is ready for implementation only when one exact Worker kind completes
