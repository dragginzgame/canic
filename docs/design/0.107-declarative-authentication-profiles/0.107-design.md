# Canic 0.107 Design: Declarative Authentication Profiles
...
- Release boundary: reinstall only. 0.107 consumes no prior-release token,
...
- Sequence: this is the 0.107 design line. It follows the
  ...
  the
  [0.105 direct transport groundwork](../0.105-cross-subnet-data-transport-groundwork/0.105-design.md)
  and the proposed
  [0.106 Coordinator Worker concept](../0.106-coordinator-workers/0.106-design.md).
...
0.107 proposes one declarative **authentication profile** as the application
...
0.107 will make authentication profiles the canonical source of login-time
...
Before this mode is implementation-approved, 0.107 must freeze:
...
0.107 does not claim to remove that initial token-signature operation. Its
...
policy. 0.107 must not create another tokenless session record, decision
...
0.107 owns declarative profile compilation, issuance, verifier projections and
client acquisition.
...
0.107 is ready for implementation only when:
