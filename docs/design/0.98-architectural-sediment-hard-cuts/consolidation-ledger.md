...
| CANIC-098-POST-ICRC103-001 | P2 | Global config can advertise ICRC-103 through the supported-standards query even though Canic has no allowance-list endpoint, required ledger metadata, or surviving generic ICRC-2 allowance authority. | FIXED in open `0.98.21` — unsupported config/render/query branches removed; the result lists only ICRC-10 and configured ICRC-21 |
