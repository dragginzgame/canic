...
| CANIC-098-POST-AUTH-001 | P2 | `root_proof_mode` accepts only `chain_key_batch`; `RootProofMode` has one variant; verification rejects an impossible alternative; canonical encoding still emits its obsolete tag at three hash positions. The removed alternative disappeared in `0.76.6`. | ACCEPT — remove the selector and tag, advance the canonical trust domains, reject all old derived material, and reprovision without a compatibility verifier; implementation pending |
