# Canic 0.108 Design: Standalone Blob Service Extraction
...
- Release boundary: reinstall only. Canic 0.108 does not migrate, import,
  adopt or decode blob state written by Canic 0.69 through 0.107.
...
- Version ownership: `0.108` is the Canic integration/removal release, not the
...
- Repository boundary: the implementation belongs in a standalone
  project. Canic 0.108 owns the Canic hard cut and the ordinary Component
...
0.108 removes blob-storage product semantics from Canic and establishes a
...
generic capability and 0.108 pauses for a separate backend-neutral Canic
...
- require Canic 0.108 to wait for Toko/IcyDB production deployment;
...
0.108 keeps the identities separate:
...
0.101 schema. 0.108 must not add a blob-specific configuration table.
...
hard cut. Canic 0.108 pins the exact standalone Wasm as an ordinary application
...
does not block Canic 0.108 closeout once the qualified standalone release and
...
0.108 removes the current blob subsystem rather than wrapping it.
...
IDs 55 through 58 become unallocated `canic-core` reserve in 0.108. They are
...
Canic 0.108 is a fresh installation. Existing Canic-embedded blob state is not
...
mixed-version operation are outside 0.108. A future service release must
...
production adoption remains downstream work and is not a Canic 0.108 release
dependency.
...
- optional Canic backup archival, mandatory client-side encryption,
  ...
  [0.110 encrypted canister-snapshot archive design](../0.110-optional-encrypted-canister-snapshot-archives/0.110-design.md);
