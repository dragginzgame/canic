...
- For each replay slot `(caller_principal, subnet_id, request_id)`, Root **MUST** bind and store the first accepted `ReplayDigest`.
...
- Replay storage identity is `ReplaySlotKey = H(caller || subnet || request_id)`.
