...
- Package version: `0.110.42`
- Release date: `2026-09-25`
- Validated source: `10d00c6d9494a45b30e66f84d1bd886c8acdd45c`
...
`a38baf91613d5aadb8434d3e3161eb68c66e10a1`; the subsequent pre-bump remote check
rejected its missing published `.38` ancestry. The merge resolution preserves
the published release metadata and the current `.39` draft/B2 work. The merge
commit and publication remain maintainer-owned.

Release preflight now refreshes remote ancestry and checks the planned tag before
expensive validation or receipt reuse, retaining the existing final pre-bump
recheck. Focused lane and remote-state fixtures cover rejection before validation,
fast eligibility and mutation, plus retained-receipt retries. The successful
complete receipt remains bound to its original source; it is not transferred to
the merged source or relabelled as validation of these automation changes.
...
or sibling mutation ran during B2 qualification. The subsequent merge resolution
retains the published `.38` release metadata together with the current `.39`
draft. The maintainer still owns the merge commit and release flow.
...
<!-- canic-release-validation: version=0.110.42 source=10d00c6d9494a45b30e66f84d1bd886c8acdd45c date=2026-09-25 gate=complete -->
