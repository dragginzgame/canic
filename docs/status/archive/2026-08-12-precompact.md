# Historical Status Through 2026-08-12
...
- Released `0.101.41` closes the cross-root Fleet-service peer requester
  ...
  the compiled requester-Spec-to-target-Spec grant. The allocation journal
  retains that identity, Registry authority and grant, and every later peer
  lifecycle step revalidates them. Same-root peers retain local Registry proof.
...
- Released `0.101.29` advances the current accepted scale-out root one canonical
  ...
  asynchronous observation, while the Coordinator retains its exact pre-call
  intent and response. The operation stops after the final install; Component
  Registry commitment, Fleet-service append, Directory confirmation and
  runtime activation remain fenced.
- Released `0.101.28` advances the current accepted scale-out root one canonical
...
- Released `0.100.26` activates that Component runtime only from its protected
...
- Released `0.100.27` atomically promotes that exactly runtime-active Component
  ...
  to the target, reconciles uncertain response through status, independently
  verifies convergence and commits the terminal root membership bit last.
  The full original activation Directory remains unchanged. Current queries
...
- Released `0.100.29` hard-cuts role-attestation issuance and retrieval from the
...
- Released `0.100.32` hard-cuts terminal Fleet catalog v1 from one
...
- Released `0.100.37` persists that decision as one exact parent-authenticated
...
- Released `0.100.41` distributes that committed revision to the `Prepared`
...
- Released `0.100.55` atomically removes one independently deleted childless leaf
...
- Released `0.100.75` adds the maintained host effect executor without a
  competing local journal. It resumes from root/Coordinator status, requires complete
  ...
  are redirected to the executor's ledger. The same release moves reference
...
- Released `0.100.89` consolidates fresh host installation's
...
- Released `0.100.90` makes Store garbage collection and physical-deletion
  ...
  workflows.
- Released `0.100.91` gives bootstrap and publication Store creation one durable
...
- Released `v0.96.0` anchors Slice A to `v0.95.10`, freezes the complete
  ...
  then-current sibling Toko mint flow read-only. That historical snapshot had
  not adopted the API and contained no old receipt rows. Released `v0.96.1`
  measures the inherited 100,000-row
  ...
  reservation, and earliest-eligibility projection. Released `v0.96.6`
...
with one ordered pass over each canonical map. Released `.4` adds the sole ordered
...
cell or stable allocation. Released `.6` hard-cuts the shared primary ceiling
from 100,000 to 1,000 and gives terminal application receipts exact bounded
removal through the existing `intent_cleanup:run` owner. The shared 32-item batch
...
application high-water is 513 physical pages, or 32.0625 MiB. Released `.7` caps
the shared resource-total map at 1,000 identities. Existing resources remain
usable at saturation, exact zero removal reopens a slot, and guarded runtime
status/inspect reports both limits with warning at 100 remaining rows and
failure at zero. Production placement keeps its separate
acknowledgement-owned removal and stores no application deadline.
The previous Toko checkout is historical and does not define current
...
- Instruction v3 rebases the three delegated-auth scenarios on active
  Registry-issued Components and expands its composite method fingerprint to
  the authoritative Fleet fixture and measured canister packages. Its 38
  non-manual tests, targeted strict Clippy and focused real activation journey
  pass; no new manual instruction-footprint measurement has been taken.
...
The cross-root Fleet-service peer requester implementation released in
`0.101.41` now has complete two-root PocketIC qualification in open
`0.101.42`, including a real inter-Canister request across two physical
application Subnets, exact retry and stale-authority, wrong-service,
ordinary-Component-forwarding and Component-Child-forwarding rejection. Next,
measure the supported root, Component, placement, service-member, plan,
Registry and Directory envelope and continue the remaining pool-funding work.
The purpose-bound runtime policy completed in released `0.101.40`: the exact
...
The Directory-prepared child now activates only from its exact retained
authority. The root reconciles an uncertain call through target status,
...
Released `0.100.55` atomically removes the independently deleted childless leaf
...
Released `0.100.75` adds the resumable host physical-root executor. Remote root
and Coordinator receipts remain the sole recovery journal; exact private status is
...
The same release advances the embedded `ic-query` host library from
...
The same release moves reference canisters onto the endpoint-level local-build
...
Released `0.100.89` makes fresh root Fleet activation consume an explicit exact
...
Released `0.100.91` replaces the remaining Store-creation Registry importer with
