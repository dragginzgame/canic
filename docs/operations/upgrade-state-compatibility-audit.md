...
| Project-local pending operation log | `crates/canic-cli/src/cycles/convert/pending.rs` writes `.canic/operations/pending.json` with log and entry schema versions, advisory-locked read/modify/write, atomic temp-file replace, directory sync, `pending_send`, and `completed` states. Tests cover concurrent reservation, write-before-send, matching pending reuse, typed terminal-response completion, and completion-write failures. | Covered by existing tests/docs | This is host/operator durability, not canister stable state. Recovery wording is documented in [Recovery and retry runbooks](recovery-retry-runbooks.md). |
