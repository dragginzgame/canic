...
| Wrong issuer or delegated-auth issuer mismatch | Root RPC errors include `delegation request caller ... must match issuer_pid ...`; delegated-auth metrics include issuer and issuer-proof mismatch labels. | None in this audit | Existing diagnostic sufficient | Wrong-issuer diagnostics are distinguishable from generic replay conflict by auth issuer wording and metrics labels. |
