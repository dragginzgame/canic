...
```bash
rg -n 'init_root_canister|init_nonroot_canister|post_upgrade_root_canister|post_upgrade_nonroot_canister|initialize_runtime_required|defer_lifecycle_required|Duration::ZERO' \
...
```bash
rg -n 'EnvOps::restore_|init_memory_registry_post_upgrade|workflow::runtime::init_|post_upgrade_.*after_memory_init|initialize_runtime_required|defer_lifecycle_required' \
...
* lifecycle owners initialize the shared runtime synchronously before fixed
  declarations, framework work, and application hooks
* core, facade, and control-plane lifecycle boundaries schedule required work
  through `TimerApi::defer_lifecycle_required`
* no direct `ic-cdk-timers` or removed `TimerOps` provider call remains
