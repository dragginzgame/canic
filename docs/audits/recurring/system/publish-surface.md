...
* `canic-backup`, `canic-host`, and `canic-cli` are published role-specific crates; their binary/README posture must clearly say so.
* Lower-level published crates are allowed to be thinner than `canic`, but not allowed to be misleading about their role.
* Root, Coordinator and Store entrypoint packages are generated by `canic-host`
  with `publish = false`; their runtime owners ship through Canic and its control
  plane. Consumers build artifacts without separate Fleet package dependencies.
...
* the published MSRV is package-contract evidence. If the internal toolchain is
  ...
  understandable.
* a generated Fleet canister artifact source must not ship an `rlib`
...
```bash
cargo package -p canic -p canic-backup -p canic-cli -p canic-control-plane -p canic-core -p canic-host -p canic-macros --locked --allow-dirty
```
