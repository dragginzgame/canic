...
- Code snapshot identifier: `3c2499a1`
...
- Delta summary: total runtime files grew `377 -> 441` and logical LOC grew
  `40156 -> 45009`; runtime enum variants grew in the root request/capability
...
- Follow-up required: **no immediate runtime split required**. Next routine
  cleanup pressure is in workflow placement/request-handler modules rather than
  the protected internal-call facade.

## Baseline Capture

| Metric | Previous | Current | Delta |
| ---- | ----: | ----: | ----: |
| Total runtime files in scope | 377 | 441 | +64 |
| Runtime logical LOC | 40156 | 45009 | +4853 |
| Non-test runtime files | 365 | 422 | +57 |
| Non-test runtime logical LOC | 37883 | 39540 | +1657 |
| Files >= 600 LOC | 6 | 3 | -3 |
...
| --- | ---: | ---: | ---: | ---: | --- | --- |
| `workflow/placement/sharding/mod.rs` | 560 | 9 | 16 | 4.46 | active placement workflow remains the largest production module and mixes allocation, topology, policy, and repair axes | Medium |
...
- `+1` request-handler and placement workflow modules remain the highest
  branch-axis pressure centers after the protected internal-call split.
...
| --- | --- | --- |
| `git rev-parse --short HEAD` | PASS | Captured `3c2499a1`. |
| `find crates/canic-core/src -name '*.rs' | wc -l` | PASS | Captured `441` files. |
| logical LOC scan over `crates/canic-core/src` | PASS | Captured `45009` non-blank/non-comment lines. |
| non-test logical LOC scan | PASS | Captured `422` files and `39540` logical LOC. |
...
2. Completed after the run: split protected internal-call endpoint descriptor,
   ...
   `api/ic/canic/`.
3. Completed after the run: update the recurring complexity template subsystem
...
4. Carry forward: keep new placement and request-handler branch axes in focused
