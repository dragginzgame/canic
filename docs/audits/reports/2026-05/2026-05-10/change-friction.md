...
- Code snapshot identifier: `0aef61ed` plus active dirty `0.33.7` metadata/list
  worktree.
...
- Delta summary: change friction is materially higher than the April baseline.
  The sampled 0.33 slices average `69.4` touched files, up from `19.25`, and
...
| Avg files touched per sampled slice | 19.25 | 69.40 | +50.15 |
| p95 files touched | 27 | 88 | +61 |
| Top gravity-well fan-in | 2 | 4 | +2 |

Notes:

- Current averages use sampled 0.33 commits `8a5814fd`, `cf24f77e`,
  `53476764`, `6ea85fdb`, and `5b474986`.
- The active dirty `0.33.7` worktree touches `22` tracked files plus new
  metadata files; it is recorded as an in-progress pressure signal but is not
  included in the committed-slice averages.
...
| --- | --- | --- |
| Feature slices sampled from recent commits | PASS | sampled the main `0.33.0`, `0.33.1`, `0.33.2`, `0.33.5`, and `0.33.6` implementation commits |
...
| Blast radius measured | PASS | sampled committed slices touched `46` to `88` files |
...
- Regressed: average sampled file blast radius rose from `19.25` to `69.40`.
...
4. Active `0.33.7` metadata/list work crosses facade, core DTO/API,
   wasm-store DID, host install/list support, and CLI table rendering.
   Evidence: dirty worktree has `22` tracked changed files plus new
   `metadata` files. The scope is justified by endpoint replacement, but it is
   a current friction signal.
...
| --- | ---: | --- |
| `crates/canic-cli/src/list/mod.rs` | 4 | high |
| `crates/canic-cli/src/lib.rs` | 3 | medium |
| `crates/canic-host/src/install_root/mod.rs` | 3 | medium |
| `crates/canic-host/src/release_set/mod.rs` | 3 | medium |
| `crates/canic-host/src/icp.rs` | 3 | medium |
...
- `+1` active metadata/list endpoint work crosses runtime, host, and CLI
  surfaces.
...
| --- | --- | --- |
| `git rev-parse --short HEAD` | PASS | captured snapshot `0aef61ed` |
| `git status --short` | PASS | confirmed dirty active `0.33.7` metadata/list worktree |
| `git log --oneline -n 30` | PASS | selected recent 0.33 implementation samples |
