...
identity, policy, guard, provenance, and execution-receipt helpers into
focused child modules. The promotion parent is now a smaller coordination
module, with provenance/execution receipt logic isolated under promotion.
...
| host deployment-truth module pressure remains persistent | persistent | true drag | Lifecycle and install-root remain the main large host follow-ups after promotion provenance/receipt helpers moved out of the promotion parent. |
...
| host deployment-truth gravity well | `deployment_truth/promotion/mod.rs`, `deployment_truth/lifecycle/mod.rs`, `install_root/mod.rs` | promotion parent pressure is lower after child-module extraction; lifecycle and install-root remain large host files | Medium |
