# Instruction Footprint Audit - 2026-06-01

## Report Preamble

- Scope: Canic instruction footprint (first `0.57` baseline, partial canister scope)
- Definition path: `docs/audits/recurring/system/instruction-footprint.md`
- Compared baseline report path: `N/A`
- Code snapshot identifier: `9bd3ad6f`
- Method tag/version: `Method V1`
- Comparability status: `partial`
- Auditor: `codex`
- Run timestamp (UTC): `2026-06-01T13:53:42Z`
- Branch: `main`
- Worktree: `dirty`
...
| --- | --- | --- | ---: | ---: | ---: | --- | --- |
| `leaf_probe` | `audit_time_probe` | `minimal-valid` | 1 | 28960 | 28960 | N/A | local-only QueryPerfSample probe |
| `leaf_probe` | `audit_env_probe` | `minimal-valid` | 1 | 30428 | 30428 | N/A | local-only QueryPerfSample probe |
| `leaf_probe` | `audit_log_probe` | `empty-page` | 1 | 294125 | 294125 | N/A | local-only QueryPerfSample probe |
| `root_probe` | `audit_subnet_registry_probe` | `representative-valid` | 1 | 73910 | 73910 | N/A | local-only QueryPerfSample probe |
| `root_probe` | `audit_subnet_state_probe` | `minimal-valid` | 1 | 30550 | 30550 | N/A | local-only QueryPerfSample probe |
| `scaling_probe` | `audit_plan_create_worker_probe` | `empty-pool` | 1 | 62179 | 62179 | N/A | local-only QueryPerfSample probe |
| `root` | `canic_request_delegation` | `fresh-shard` | 1 | 832477 | 832477 | N/A |  |
| `test` | `test` | `minimal-valid` | 1 | 752 | 752 | N/A |  |
| `root` | `canic_response_capability_v1` | `cycles-request` | 1 | 649229 | 649229 | N/A |  |
| `root` | `canic_template_stage_manifest_admin` | `single-chunk` | 1 | 408442 | 408442 | N/A |  |
| `root` | `canic_template_prepare_admin` | `single-chunk` | 1 | 187784 | 187784 | N/A |  |
| `root` | `canic_template_publish_chunk_admin` | `single-chunk` | 1 | 333256 | 333256 | N/A |  |
...
| --- | --- | --- | ---: | ---: | ---: |
| `root:canic_template_publish_chunk_admin:single-chunk` | `canic_control_plane::storage::stable::template::chunked` | `chunk_store_insert` | 1 | 137710 | 137710 |
| `root:canic_template_publish_chunk_admin:single-chunk` | `canic_control_plane::ops::storage::template::chunked` | `publish_stage_upsert_chunk` | 1 | 6845 | 6845 |
| `root:canic_template_publish_chunk_admin:single-chunk` | `canic_control_plane::storage::stable::template::chunked` | `chunk_store_accounting` | 1 | 6453 | 6453 |
...
| --- | --- | ---: | --- | --- |
| 1 | `root:canic_request_delegation:fresh-shard` | 832477 | Root delegated auth request path | [root endpoints](/home/adam/projects/canic/crates/canic/src/macros/endpoints/root.rs), [auth workflow](/home/adam/projects/canic/crates/canic-core/src/workflow/auth.rs) |
| 2 | `root:canic_response_capability_v1:request-cycles-fresh` | 649229 | Root dispatcher plus replay/capability workflow | [request handler](/home/adam/projects/canic/crates/canic-core/src/workflow/rpc/request/handler/mod.rs), [replay workflow](/home/adam/projects/canic/crates/canic-core/src/workflow/rpc/request/handler/replay.rs) |
| 3 | `root:canic_template_stage_manifest_admin:single-chunk` | 408442 | Root template publication admin path | [root endpoints](/home/adam/projects/canic/crates/canic/src/macros/endpoints/root.rs), [template storage ops](/home/adam/projects/canic/crates/canic-control-plane/src/ops/storage/template/chunked.rs) |
...
| Highest sampled endpoint currently highest-cost | WARN | `root:canic_request_delegation:fresh-shard` averages 832477 local instructions in this first baseline. |
...
| `canic_metrics(MetricsKind::Runtime, PageRequest { limit=512, offset=0 })` | PASS | Update scenarios were sampled before/after through persisted perf rows, and query scenarios used local-only `QueryPerfSample` probe endpoints because query-side perf rows are not committed; normalized rows saved under `/home/adam/projects/canic/docs/audits/reports/2026-06/2026-06-01/artifacts/instruction-footprint/perf-rows.json`. |
| `repo checkpoint scan` | PASS | Found 56 checkpoint call sites. |
| `checkpoint delta capture` | PASS | 4 non-zero checkpoint delta rows were captured under `/home/adam/projects/canic/docs/audits/reports/2026-06/2026-06-01/artifacts/instruction-footprint/checkpoint-deltas.json`. |
...
2. Owner boundary: `shared update hotspots`
   Action: compare `root::canic_request_delegation`, `root::canic_response_capability_v1`, and the local `test::test` update floor before/after any shared-runtime cleanup, using this report as the `0.57` baseline.
