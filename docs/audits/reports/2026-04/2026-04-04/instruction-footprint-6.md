...
- Compared baseline report path: `docs/audits/reports/2026-04/2026-04-04/instruction-footprint-5.md`
- Code snapshot identifier: `4d32edea`
- Method tag/version: `Method V1`
- Comparability status: `partial`
- Auditor: `codex`
- Run timestamp (UTC): `2026-04-04T12:23:03Z`
- Branch: `main`
- Worktree: `dirty`
...
- Deferred from this baseline: direct verifier-side auth confirmation is still unsampled; this run now covers shared queries plus delegated auth issuance, replay/cycles, scaling worker creation, sharding account creation, and root template admin updates.
...
Interpretation: query visibility and stage attribution are now working for sampled updates. The remaining audit risk is mostly first-run comparability (`N/A` baseline deltas) plus one still-unsampled direct verifier-side auth lane, not missing observability on the measured flows.
...
| `canic_metrics(MetricsKind::Perf, PageRequest { limit=512, offset=0 })` | PASS | Update scenarios were sampled before/after through persisted perf rows, and query scenarios used same-call local-only probe endpoints; normalized rows saved under `/home/adam/projects/canic/docs/audits/reports/2026-04/2026-04-04/artifacts/instruction-footprint-6/perf-rows.json`. |
| `repo checkpoint scan` | PASS | Found 51 checkpoint call sites. |
| `checkpoint delta capture` | PASS | 34 non-zero checkpoint delta rows were captured under `/home/adam/projects/canic/docs/audits/reports/2026-04/2026-04-04/artifacts/instruction-footprint-6/checkpoint-deltas.json`. |
...
1. Owner boundary: `flow observability`
   Action: add one direct verifier-side auth update scenario so delegated issuance and verifier confirmation both appear as measured endpoints in the matrix.
