...
- Compared baseline report path: `docs/audits/reports/2026-04/2026-04-04/instruction-footprint-4.md`
- Code snapshot identifier: `4d32edea`
- Method tag/version: `Method V1`
- Comparability status: `partial`
- Auditor: `codex`
- Run timestamp (UTC): `2026-04-04T12:19:01Z`
- Branch: `main`
- Worktree: `dirty`
- Execution environment: `PocketIC`
- Target canisters in scope: `app` `root` `scale_hub` `test` `user_hub`
- Target endpoints/flows in scope: `canic_env` `canic_log` `canic_request_delegation` `canic_response_capability_v1` `canic_subnet_registry` `canic_subnet_state` `canic_template_prepare_admin` `canic_template_publish_chunk_admin` `canic_template_stage_manifest_admin` `canic_time` `create_account` `create_worker` `plan_create_worker` `test`
- Deferred from this baseline: auth issuance/verification updates are not sampled yet; this run now covers shared queries plus replay/cycles, scaling worker creation, sharding account creation, and root template admin updates.
...
| `root` | `canic_subnet_registry` | `representative-valid` | 1 | 181121 | 181121 | N/A | same-call local-only perf probe |
| `root` | `canic_subnet_state` | `minimal-valid` | 1 | 20867 | 20867 | N/A | same-call local-only perf probe |
...
| `root` | `canic_request_delegation` | `fresh-shard` | 1 | 5493462 | 5493462 | N/A |  |
| `test` | `test` | `minimal-valid` | 1 | 421 | 421 | N/A |  |
| `root` | `canic_response_capability_v1` | `cycles-request` | 1 | 2211636 | 2211636 | N/A |  |
| `root` | `canic_template_stage_manifest_admin` | `single-chunk` | 1 | 587629 | 587629 | N/A |  |
| `root` | `canic_template_prepare_admin` | `single-chunk` | 1 | 222543 | 222543 | N/A |  |
| `root` | `canic_template_publish_chunk_admin` | `single-chunk` | 1 | 18224516 | 18224516 | N/A |  |
...
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler` | `commit_replay` | 1 | 1151608 | 1151608 |
| `root:canic_response_capability_v1:request-cycles-fresh` | `canic_core::workflow::rpc::request::handler` | `commit_replay` | 1 | 1023826 | 1023826 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `issue_proof` | 1 | 900300 | 900300 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::replay` | `prepare_replay_input` | 1 | 779301 | 779301 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler` | `execute_capability` | 1 | 724922 | 724922 |
| `root:canic_response_capability_v1:request-cycles-fresh` | `canic_core::workflow::rpc::request::handler::replay` | `prepare_replay_input` | 1 | 672799 | 672799 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `prepare_call` | 2 | 642761 | 321380 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `decode_response` | 2 | 521632 | 260816 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::replay` | `evaluate_replay` | 1 | 378977 | 378977 |
| `root:canic_response_capability_v1:request-cycles-fresh` | `canic_core::workflow::rpc::request::handler` | `execute_capability` | 1 | 196121 | 196121 |
...
| --- | --- | ---: | --- | --- |
| 1 | `root:canic_template_publish_chunk_admin:single-chunk` | 18224516 | Shared runtime surface | [endpoints](/home/adam/projects/canic/crates/canic/src/macros/endpoints.rs) |
| 2 | `root:canic_request_delegation:fresh-shard` | 5493462 | Shared runtime surface | [endpoints](/home/adam/projects/canic/crates/canic/src/macros/endpoints.rs) |
...
| Highest sampled endpoint currently highest-cost | WARN | `root:canic_template_publish_chunk_admin:single-chunk` averages 18224516 local instructions in this first baseline. |
...
Interpretation: query visibility and stage attribution are now working for sampled updates. The remaining audit risk is mostly first-run comparability (`N/A` baseline deltas) plus one still-unsampled auth lane, not missing observability on the measured flows.
...
| `canic_metrics(MetricsKind::Perf, PageRequest { limit=512, offset=0 })` | PASS | Update scenarios were sampled before/after through persisted perf rows, and query scenarios used same-call local-only probe endpoints; normalized rows saved under `/home/adam/projects/canic/docs/audits/reports/2026-04/2026-04-04/artifacts/instruction-footprint-5/perf-rows.json`. |
| `repo checkpoint scan` | PASS | Found 51 checkpoint call sites. |
| `checkpoint delta capture` | PASS | 34 non-zero checkpoint delta rows were captured under `/home/adam/projects/canic/docs/audits/reports/2026-04/2026-04-04/artifacts/instruction-footprint-5/checkpoint-deltas.json`. |
...
1. Owner boundary: `flow observability`
   Action: add one auth issuance/verification update scenario so every critical checkpointed flow has at least one measured endpoint in the matrix.
