...
- Compared baseline report path: `docs/audits/reports/2026-04/2026-04-06/instruction-footprint-17.md`
- Code snapshot identifier: `9fc4ec89`
- Method tag/version: `Method V1`
- Comparability status: `partial`
- Auditor: `codex`
- Run timestamp (UTC): `2026-04-06T14:03:26Z`
- Branch: `main`
- Worktree: `dirty`
...
| --- | --- | --- |
| Scenario manifest recorded | PASS | `artifacts/instruction-footprint-18/scenario-manifest.json` captures the scenario identity tuple for every sampled endpoint. |
| Normalized perf rows recorded | PASS | `artifacts/instruction-footprint-18/perf-rows.json` stores canonical endpoint rows with count and total local instructions. |
| Checkpoint deltas recorded | PASS | `artifacts/instruction-footprint-18/checkpoint-deltas.json` stores non-zero per-scenario checkpoint rows. |
...
| Flow checkpoint coverage scanned | PASS | `artifacts/instruction-footprint-18/flow-checkpoints.log` records the current repo scan result. |
...
| --- | --- | --- | ---: | ---: | ---: | --- | --- |
| `audit_leaf_probe` | `audit_time_probe` | `minimal-valid` | 1 | 33704 | 33704 | N/A | same-call local-only perf probe |
| `audit_leaf_probe` | `audit_env_probe` | `minimal-valid` | 1 | 35350 | 35350 | N/A | same-call local-only perf probe |
| `audit_leaf_probe` | `audit_log_probe` | `empty-page` | 1 | 312927 | 312927 | N/A | same-call local-only perf probe |
| `audit_root_probe` | `audit_subnet_registry_probe` | `representative-valid` | 1 | 76443 | 76443 | N/A | same-call local-only perf probe |
| `audit_root_probe` | `audit_subnet_state_probe` | `minimal-valid` | 1 | 32001 | 32001 | N/A | same-call local-only perf probe |
| `audit_scaling_probe` | `audit_plan_create_worker_probe` | `empty-pool` | 1 | 64501 | 64501 | N/A | same-call local-only perf probe |
| `root` | `canic_request_delegation` | `fresh-shard` | 1 | 1682331 | 1682331 | N/A |  |
| `test` | `test` | `minimal-valid` | 1 | 817 | 817 | N/A |  |
| `root` | `canic_response_capability_v1` | `cycles-request` | 1 | 489511 | 489511 | N/A |  |
| `root` | `canic_template_stage_manifest_admin` | `single-chunk` | 1 | 415911 | 415911 | N/A |  |
| `root` | `canic_template_prepare_admin` | `single-chunk` | 1 | 187505 | 187505 | N/A |  |
| `root` | `canic_template_publish_chunk_admin` | `single-chunk` | 1 | 337142 | 337142 | N/A |  |
...
- `crates/canic-core/src/workflow/rpc/request/handler/execute.rs:165:crate::perf!("cache_root_verifier_proof");`
...
| --- | --- | --- | ---: | ---: | ---: |
| `root:canic_request_delegation:fresh-shard` | `canic_core::ops::auth::delegation` | `sign_cert` | 1 | 335844 | 335844 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `encode_install_request` | 1 | 296609 | 296609 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `decode_response` | 1 | 213578 | 213578 |
| `root:canic_template_publish_chunk_admin:single-chunk` | `canic_control_plane::storage::stable::template::chunked` | `chunk_store_insert` | 1 | 138861 | 138861 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::replay` | `prepare_replay_input` | 1 | 138772 | 138772 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::execute` | `cache_root_verifier_proof` | 1 | 99956 | 99956 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `prepare_call` | 1 | 69459 | 69459 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::replay` | `evaluate_replay` | 1 | 64132 | 64132 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::ops::auth::delegation` | `hash_cert` | 1 | 63502 | 63502 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `execute_call` | 1 | 59164 | 59164 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `push_verifiers` | 1 | 58202 | 58202 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler` | `commit_replay` | 1 | 49484 | 49484 |
...
| --- | --- | ---: | --- | --- |
| 1 | `root:canic_request_delegation:fresh-shard` | 1682331 | Shared runtime surface | [endpoints](/home/adam/projects/canic/crates/canic/src/macros/endpoints.rs) |
| 2 | `root:canic_response_capability_v1:request-cycles-fresh` | 489511 | Root dispatcher plus replay/capability workflow | [request handler](/home/adam/projects/canic/crates/canic-core/src/workflow/rpc/request/handler/mod.rs), [replay workflow](/home/adam/projects/canic/crates/canic-core/src/workflow/rpc/request/handler/replay.rs) |
| 3 | `root:canic_template_stage_manifest_admin:single-chunk` | 415911 | Shared runtime surface | [endpoints](/home/adam/projects/canic/crates/canic/src/macros/endpoints.rs) |
...
| Highest sampled endpoint currently highest-cost | WARN | `root:canic_request_delegation:fresh-shard` averages 1682331 local instructions in this first baseline. |
...
| `canic_metrics(MetricsKind::Perf, PageRequest { limit=512, offset=0 })` | PASS | Update scenarios were sampled before/after through persisted perf rows, and query scenarios used same-call local-only probe endpoints because query-side perf rows are not committed; normalized rows saved under `/home/adam/projects/canic/docs/audits/reports/2026-04/2026-04-06/artifacts/instruction-footprint-18/perf-rows.json`. |
| `repo checkpoint scan` | PASS | Found 69 checkpoint call sites. |
| `checkpoint delta capture` | PASS | 25 non-zero checkpoint delta rows were captured under `/home/adam/projects/canic/docs/audits/reports/2026-04/2026-04-06/artifacts/instruction-footprint-18/checkpoint-deltas.json`. |
...
- [instruction-footprint-18.md](./instruction-footprint-18.md)
- [scenario-manifest.json](artifacts/instruction-footprint-18/scenario-manifest.json)
- [perf-rows.json](artifacts/instruction-footprint-18/perf-rows.json)
- [endpoint-matrix.tsv](artifacts/instruction-footprint-18/endpoint-matrix.tsv)
- [checkpoint-deltas.json](artifacts/instruction-footprint-18/checkpoint-deltas.json)
- [flow-checkpoints.log](artifacts/instruction-footprint-18/flow-checkpoints.log)
- [checkpoint-coverage-gaps.json](artifacts/instruction-footprint-18/checkpoint-coverage-gaps.json)
- [verification-readout.md](artifacts/instruction-footprint-18/verification-readout.md)
- [method.json](artifacts/instruction-footprint-18/method.json)
- [environment.json](artifacts/instruction-footprint-18/environment.json)
