...
- Compared baseline report path: `docs/audits/reports/2026-04/2026-04-06/instruction-footprint-12.md`
- Code snapshot identifier: `9fc4ec89`
- Method tag/version: `Method V1`
- Comparability status: `partial`
- Auditor: `codex`
- Run timestamp (UTC): `2026-04-06T13:37:05Z`
- Branch: `main`
- Worktree: `dirty`
...
| --- | --- | --- |
| Scenario manifest recorded | PASS | `artifacts/instruction-footprint-13/scenario-manifest.json` captures the scenario identity tuple for every sampled endpoint. |
| Normalized perf rows recorded | PASS | `artifacts/instruction-footprint-13/perf-rows.json` stores canonical endpoint rows with count and total local instructions. |
| Checkpoint deltas recorded | PASS | `artifacts/instruction-footprint-13/checkpoint-deltas.json` stores non-zero per-scenario checkpoint rows. |
...
| Flow checkpoint coverage scanned | PASS | `artifacts/instruction-footprint-13/flow-checkpoints.log` records the current repo scan result. |
...
| --- | --- | --- | ---: | ---: | ---: | --- | --- |
| `audit_leaf_probe` | `audit_time_probe` | `minimal-valid` | 1 | 33418 | 33418 | N/A | same-call local-only perf probe |
| `audit_leaf_probe` | `audit_env_probe` | `minimal-valid` | 1 | 35067 | 35067 | N/A | same-call local-only perf probe |
| `audit_leaf_probe` | `audit_log_probe` | `empty-page` | 1 | 315062 | 315062 | N/A | same-call local-only perf probe |
| `audit_root_probe` | `audit_subnet_registry_probe` | `representative-valid` | 1 | 77054 | 77054 | N/A | same-call local-only perf probe |
| `audit_root_probe` | `audit_subnet_state_probe` | `minimal-valid` | 1 | 31975 | 31975 | N/A | same-call local-only perf probe |
| `audit_scaling_probe` | `audit_plan_create_worker_probe` | `empty-pool` | 1 | 63929 | 63929 | N/A | same-call local-only perf probe |
| `root` | `canic_request_delegation` | `fresh-shard` | 1 | 1679415 | 1679415 | N/A |  |
| `test` | `test` | `minimal-valid` | 1 | 891 | 891 | N/A |  |
| `root` | `canic_response_capability_v1` | `cycles-request` | 1 | 490744 | 490744 | N/A |  |
| `root` | `canic_template_stage_manifest_admin` | `single-chunk` | 1 | 416859 | 416859 | N/A |  |
| `root` | `canic_template_prepare_admin` | `single-chunk` | 1 | 185149 | 185149 | N/A |  |
| `root` | `canic_template_publish_chunk_admin` | `single-chunk` | 1 | 337207 | 337207 | N/A |  |
...
- `crates/canic-core/src/workflow/rpc/request/handler/execute.rs:158:crate::perf!("cache_root_verifier_keys");`
- `crates/canic-core/src/workflow/rpc/request/handler/execute.rs:166:crate::perf!("cache_root_verifier_proof");`
...
| --- | --- | --- | ---: | ---: | ---: |
| `root:canic_request_delegation:fresh-shard` | `canic_core::ops::auth::delegation` | `sign_cert` | 1 | 332133 | 332133 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `encode_install_request` | 1 | 296292 | 296292 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `decode_response` | 1 | 214601 | 214601 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::replay` | `prepare_replay_input` | 1 | 139647 | 139647 |
| `root:canic_template_publish_chunk_admin:single-chunk` | `canic_control_plane::storage::stable::template::chunked` | `chunk_store_insert` | 1 | 139246 | 139246 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::execute` | `cache_root_verifier_proof` | 1 | 100293 | 100293 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `prepare_call` | 1 | 67360 | 67360 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::replay` | `evaluate_replay` | 1 | 64131 | 64131 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::ops::auth::delegation` | `hash_cert` | 1 | 64130 | 64130 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `execute_call` | 1 | 58955 | 58955 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `push_verifiers` | 1 | 58072 | 58072 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler` | `commit_replay` | 1 | 49434 | 49434 |
...
| --- | --- | ---: | --- | --- |
| 1 | `root:canic_request_delegation:fresh-shard` | 1679415 | Shared runtime surface | [endpoints](/home/adam/projects/canic/crates/canic/src/macros/endpoints.rs) |
| 2 | `root:canic_response_capability_v1:request-cycles-fresh` | 490744 | Root dispatcher plus replay/capability workflow | [request handler](/home/adam/projects/canic/crates/canic-core/src/workflow/rpc/request/handler/mod.rs), [replay workflow](/home/adam/projects/canic/crates/canic-core/src/workflow/rpc/request/handler/replay.rs) |
| 3 | `root:canic_template_stage_manifest_admin:single-chunk` | 416859 | Shared runtime surface | [endpoints](/home/adam/projects/canic/crates/canic/src/macros/endpoints.rs) |
...
| --- | --- | --- |
| Flow checkpoint coverage present | INFO | Current repo scan found 69 `perf!` call sites under `crates/`. |
| Highest sampled endpoint currently highest-cost | WARN | `root:canic_request_delegation:fresh-shard` averages 1679415 local instructions in this first baseline. |
...
| `canic_metrics(MetricsKind::Perf, PageRequest { limit=512, offset=0 })` | PASS | Update scenarios were sampled before/after through persisted perf rows, and query scenarios used same-call local-only probe endpoints because query-side perf rows are not committed; normalized rows saved under `/home/adam/projects/canic/docs/audits/reports/2026-04/2026-04-06/artifacts/instruction-footprint-13/perf-rows.json`. |
| `repo checkpoint scan` | PASS | Found 69 checkpoint call sites. |
| `checkpoint delta capture` | PASS | 25 non-zero checkpoint delta rows were captured under `/home/adam/projects/canic/docs/audits/reports/2026-04/2026-04-06/artifacts/instruction-footprint-13/checkpoint-deltas.json`. |
...
- [instruction-footprint-13.md](./instruction-footprint-13.md)
- [scenario-manifest.json](artifacts/instruction-footprint-13/scenario-manifest.json)
- [perf-rows.json](artifacts/instruction-footprint-13/perf-rows.json)
- [endpoint-matrix.tsv](artifacts/instruction-footprint-13/endpoint-matrix.tsv)
- [checkpoint-deltas.json](artifacts/instruction-footprint-13/checkpoint-deltas.json)
- [flow-checkpoints.log](artifacts/instruction-footprint-13/flow-checkpoints.log)
- [checkpoint-coverage-gaps.json](artifacts/instruction-footprint-13/checkpoint-coverage-gaps.json)
- [verification-readout.md](artifacts/instruction-footprint-13/verification-readout.md)
- [method.json](artifacts/instruction-footprint-13/method.json)
- [environment.json](artifacts/instruction-footprint-13/environment.json)
