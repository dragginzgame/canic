...
- Compared baseline report path: `docs/audits/reports/2026-04/2026-04-06/instruction-footprint-9.md`
- Code snapshot identifier: `9fc4ec89`
- Method tag/version: `Method V1`
- Comparability status: `partial`
- Auditor: `codex`
- Run timestamp (UTC): `2026-04-06T13:11:58Z`
- Branch: `main`
- Worktree: `dirty`
...
| --- | --- | --- |
| Scenario manifest recorded | PASS | `artifacts/instruction-footprint-10/scenario-manifest.json` captures the scenario identity tuple for every sampled endpoint. |
| Normalized perf rows recorded | PASS | `artifacts/instruction-footprint-10/perf-rows.json` stores canonical endpoint rows with count and total local instructions. |
| Checkpoint deltas recorded | PASS | `artifacts/instruction-footprint-10/checkpoint-deltas.json` stores non-zero per-scenario checkpoint rows. |
...
| Flow checkpoint coverage scanned | PASS | `artifacts/instruction-footprint-10/flow-checkpoints.log` records the current repo scan result. |
...
| --- | --- | --- | ---: | ---: | ---: | --- | --- |
| `audit_leaf_probe` | `audit_time_probe` | `minimal-valid` | 1 | 33698 | 33698 | N/A | same-call local-only perf probe |
| `audit_leaf_probe` | `audit_env_probe` | `minimal-valid` | 1 | 35344 | 35344 | N/A | same-call local-only perf probe |
| `audit_leaf_probe` | `audit_log_probe` | `empty-page` | 1 | 315266 | 315266 | N/A | same-call local-only perf probe |
| `audit_root_probe` | `audit_subnet_registry_probe` | `representative-valid` | 1 | 76493 | 76493 | N/A | same-call local-only perf probe |
| `audit_root_probe` | `audit_subnet_state_probe` | `minimal-valid` | 1 | 31998 | 31998 | N/A | same-call local-only perf probe |
| `audit_scaling_probe` | `audit_plan_create_worker_probe` | `empty-pool` | 1 | 64495 | 64495 | N/A | same-call local-only perf probe |
| `root` | `canic_request_delegation` | `fresh-shard` | 1 | 1723474 | 1723474 | N/A |  |
| `test` | `test` | `minimal-valid` | 1 | 819 | 819 | N/A |  |
| `root` | `canic_response_capability_v1` | `cycles-request` | 1 | 492195 | 492195 | N/A |  |
| `root` | `canic_template_stage_manifest_admin` | `single-chunk` | 1 | 415836 | 415836 | N/A |  |
| `root` | `canic_template_prepare_admin` | `single-chunk` | 1 | 188749 | 188749 | N/A |  |
| `root` | `canic_template_publish_chunk_admin` | `single-chunk` | 1 | 336879 | 336879 | N/A |  |
...
| --- | --- | --- | ---: | ---: | ---: |
| `root:canic_request_delegation:fresh-shard` | `canic_core::ops::auth::delegation` | `sign_cert` | 1 | 337635 | 337635 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `encode_install_request` | 1 | 298785 | 298785 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `decode_response` | 1 | 214308 | 214308 |
| `root:canic_template_publish_chunk_admin:single-chunk` | `canic_control_plane::storage::stable::template::chunked` | `chunk_store_insert` | 1 | 139653 | 139653 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::replay` | `prepare_replay_input` | 1 | 139395 | 139395 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::execute` | `cache_root_verifier_proof` | 1 | 136098 | 136098 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `prepare_call` | 1 | 68049 | 68049 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::replay` | `evaluate_replay` | 1 | 64370 | 64370 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::ops::auth::delegation` | `hash_cert` | 1 | 63972 | 63972 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `execute_call` | 1 | 61174 | 61174 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `push_verifiers` | 1 | 57825 | 57825 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler` | `commit_replay` | 1 | 49419 | 49419 |
...
| --- | --- | ---: | --- | --- |
| 1 | `root:canic_request_delegation:fresh-shard` | 1723474 | Shared runtime surface | [endpoints](/home/adam/projects/canic/crates/canic/src/macros/endpoints.rs) |
| 2 | `root:canic_response_capability_v1:request-cycles-fresh` | 492195 | Root dispatcher plus replay/capability workflow | [request handler](/home/adam/projects/canic/crates/canic-core/src/workflow/rpc/request/handler/mod.rs), [replay workflow](/home/adam/projects/canic/crates/canic-core/src/workflow/rpc/request/handler/replay.rs) |
| 3 | `root:canic_template_stage_manifest_admin:single-chunk` | 415836 | Shared runtime surface | [endpoints](/home/adam/projects/canic/crates/canic/src/macros/endpoints.rs) |
...
| Highest sampled endpoint currently highest-cost | WARN | `root:canic_request_delegation:fresh-shard` averages 1723474 local instructions in this first baseline. |
...
| `canic_metrics(MetricsKind::Perf, PageRequest { limit=512, offset=0 })` | PASS | Update scenarios were sampled before/after through persisted perf rows, and query scenarios used same-call local-only probe endpoints because query-side perf rows are not committed; normalized rows saved under `/home/adam/projects/canic/docs/audits/reports/2026-04/2026-04-06/artifacts/instruction-footprint-10/perf-rows.json`. |
| `repo checkpoint scan` | PASS | Found 71 checkpoint call sites. |
| `checkpoint delta capture` | PASS | 25 non-zero checkpoint delta rows were captured under `/home/adam/projects/canic/docs/audits/reports/2026-04/2026-04-06/artifacts/instruction-footprint-10/checkpoint-deltas.json`. |
...
- [instruction-footprint-10.md](./instruction-footprint-10.md)
- [scenario-manifest.json](artifacts/instruction-footprint-10/scenario-manifest.json)
- [perf-rows.json](artifacts/instruction-footprint-10/perf-rows.json)
- [endpoint-matrix.tsv](artifacts/instruction-footprint-10/endpoint-matrix.tsv)
- [checkpoint-deltas.json](artifacts/instruction-footprint-10/checkpoint-deltas.json)
- [flow-checkpoints.log](artifacts/instruction-footprint-10/flow-checkpoints.log)
- [checkpoint-coverage-gaps.json](artifacts/instruction-footprint-10/checkpoint-coverage-gaps.json)
- [verification-readout.md](artifacts/instruction-footprint-10/verification-readout.md)
- [method.json](artifacts/instruction-footprint-10/method.json)
- [environment.json](artifacts/instruction-footprint-10/environment.json)
