...
- Compared baseline report path: `docs/audits/reports/2026-04/2026-04-06/instruction-footprint-18.md`
- Code snapshot identifier: `9d276513`
- Method tag/version: `Method V1`
- Comparability status: `partial`
- Auditor: `codex`
- Run timestamp (UTC): `2026-04-06T14:33:49Z`
- Branch: `main`
- Worktree: `dirty`
...
| --- | --- | --- |
| Scenario manifest recorded | PASS | `artifacts/instruction-footprint-19/scenario-manifest.json` captures the scenario identity tuple for every sampled endpoint. |
| Normalized perf rows recorded | PASS | `artifacts/instruction-footprint-19/perf-rows.json` stores canonical endpoint rows with count and total local instructions. |
| Checkpoint deltas recorded | PASS | `artifacts/instruction-footprint-19/checkpoint-deltas.json` stores non-zero per-scenario checkpoint rows. |
...
| Flow checkpoint coverage scanned | PASS | `artifacts/instruction-footprint-19/flow-checkpoints.log` records the current repo scan result. |
...
| --- | --- | --- | ---: | ---: | ---: | --- | --- |
| `audit_leaf_probe` | `audit_time_probe` | `minimal-valid` | 1 | 34668 | 34668 | N/A | same-call local-only perf probe |
| `audit_leaf_probe` | `audit_env_probe` | `minimal-valid` | 1 | 36320 | 36320 | N/A | same-call local-only perf probe |
| `audit_leaf_probe` | `audit_log_probe` | `empty-page` | 1 | 314764 | 314764 | N/A | same-call local-only perf probe |
| `audit_root_probe` | `audit_subnet_registry_probe` | `representative-valid` | 1 | 76585 | 76585 | N/A | same-call local-only perf probe |
| `audit_root_probe` | `audit_subnet_state_probe` | `minimal-valid` | 1 | 32261 | 32261 | N/A | same-call local-only perf probe |
| `audit_scaling_probe` | `audit_plan_create_worker_probe` | `empty-pool` | 1 | 64549 | 64549 | N/A | same-call local-only perf probe |
| `root` | `canic_request_delegation` | `fresh-shard` | 1 | 1695749 | 1695749 | N/A |  |
| `test` | `test` | `minimal-valid` | 1 | 817 | 817 | N/A |  |
| `root` | `canic_response_capability_v1` | `cycles-request` | 1 | 488780 | 488780 | N/A |  |
| `root` | `canic_template_stage_manifest_admin` | `single-chunk` | 1 | 416789 | 416789 | N/A |  |
| `root` | `canic_template_prepare_admin` | `single-chunk` | 1 | 187499 | 187499 | N/A |  |
| `root` | `canic_template_publish_chunk_admin` | `single-chunk` | 1 | 332061 | 332061 | N/A |  |
...
| --- | --- | --- | ---: | ---: | ---: |
| `root:canic_request_delegation:fresh-shard` | `canic_core::ops::auth::delegation` | `sign_cert` | 1 | 336026 | 336026 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `encode_install_request` | 1 | 297458 | 297458 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `decode_response` | 1 | 213153 | 213153 |
| `root:canic_template_publish_chunk_admin:single-chunk` | `canic_control_plane::storage::stable::template::chunked` | `chunk_store_insert` | 1 | 139207 | 139207 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::capability` | `hash_replay_delegation_cert` | 1 | 124760 | 124760 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::execute` | `cache_root_verifier_upsert` | 1 | 99491 | 99491 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `prepare_call` | 1 | 66302 | 66302 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::replay` | `evaluate_replay` | 1 | 64352 | 64352 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::ops::auth::delegation` | `hash_cert` | 1 | 63537 | 63537 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `execute_call` | 1 | 62975 | 62975 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `push_verifiers` | 1 | 57244 | 57244 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler` | `commit_replay` | 1 | 49718 | 49718 |
...
| --- | --- | ---: | --- | --- |
| 1 | `root:canic_request_delegation:fresh-shard` | 1695749 | Shared runtime surface | [endpoints](/home/adam/projects/canic/crates/canic/src/macros/endpoints.rs) |
| 2 | `root:canic_response_capability_v1:request-cycles-fresh` | 488780 | Root dispatcher plus replay/capability workflow | [request handler](/home/adam/projects/canic/crates/canic-core/src/workflow/rpc/request/handler/mod.rs), [replay workflow](/home/adam/projects/canic/crates/canic-core/src/workflow/rpc/request/handler/replay.rs) |
| 3 | `root:canic_template_stage_manifest_admin:single-chunk` | 416789 | Shared runtime surface | [endpoints](/home/adam/projects/canic/crates/canic/src/macros/endpoints.rs) |
...
| --- | --- | --- |
| Flow checkpoint coverage present | INFO | Current repo scan found 72 `perf!` call sites under `crates/`. |
| Highest sampled endpoint currently highest-cost | WARN | `root:canic_request_delegation:fresh-shard` averages 1695749 local instructions in this first baseline. |
...
| `canic_metrics(MetricsKind::Perf, PageRequest { limit=512, offset=0 })` | PASS | Update scenarios were sampled before/after through persisted perf rows, and query scenarios used same-call local-only probe endpoints because query-side perf rows are not committed; normalized rows saved under `/home/adam/projects/canic/docs/audits/reports/2026-04/2026-04-06/artifacts/instruction-footprint-19/perf-rows.json`. |
| `repo checkpoint scan` | PASS | Found 72 checkpoint call sites. |
| `checkpoint delta capture` | PASS | 28 non-zero checkpoint delta rows were captured under `/home/adam/projects/canic/docs/audits/reports/2026-04/2026-04-06/artifacts/instruction-footprint-19/checkpoint-deltas.json`. |
...
- [instruction-footprint-19.md](./instruction-footprint-19.md)
- [scenario-manifest.json](artifacts/instruction-footprint-19/scenario-manifest.json)
- [perf-rows.json](artifacts/instruction-footprint-19/perf-rows.json)
- [endpoint-matrix.tsv](artifacts/instruction-footprint-19/endpoint-matrix.tsv)
- [checkpoint-deltas.json](artifacts/instruction-footprint-19/checkpoint-deltas.json)
- [flow-checkpoints.log](artifacts/instruction-footprint-19/flow-checkpoints.log)
- [checkpoint-coverage-gaps.json](artifacts/instruction-footprint-19/checkpoint-coverage-gaps.json)
- [verification-readout.md](artifacts/instruction-footprint-19/verification-readout.md)
- [method.json](artifacts/instruction-footprint-19/method.json)
- [environment.json](artifacts/instruction-footprint-19/environment.json)
