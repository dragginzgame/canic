...
- Compared baseline report path: `docs/audits/reports/2026-04/2026-04-06/instruction-footprint-13.md`
- Code snapshot identifier: `9fc4ec89`
- Method tag/version: `Method V1`
- Comparability status: `partial`
- Auditor: `codex`
- Run timestamp (UTC): `2026-04-06T13:42:40Z`
- Branch: `main`
- Worktree: `dirty`
...
| --- | --- | --- |
| Scenario manifest recorded | PASS | `artifacts/instruction-footprint-14/scenario-manifest.json` captures the scenario identity tuple for every sampled endpoint. |
| Normalized perf rows recorded | PASS | `artifacts/instruction-footprint-14/perf-rows.json` stores canonical endpoint rows with count and total local instructions. |
| Checkpoint deltas recorded | PASS | `artifacts/instruction-footprint-14/checkpoint-deltas.json` stores non-zero per-scenario checkpoint rows. |
...
| Flow checkpoint coverage scanned | PASS | `artifacts/instruction-footprint-14/flow-checkpoints.log` records the current repo scan result. |
...
| --- | --- | --- | ---: | ---: | ---: | --- | --- |
| `audit_leaf_probe` | `audit_time_probe` | `minimal-valid` | 1 | 33664 | 33664 | N/A | same-call local-only perf probe |
| `audit_leaf_probe` | `audit_env_probe` | `minimal-valid` | 1 | 35316 | 35316 | N/A | same-call local-only perf probe |
| `audit_leaf_probe` | `audit_log_probe` | `empty-page` | 1 | 313457 | 313457 | N/A | same-call local-only perf probe |
| `audit_root_probe` | `audit_subnet_registry_probe` | `representative-valid` | 1 | 76707 | 76707 | N/A | same-call local-only perf probe |
| `audit_root_probe` | `audit_subnet_state_probe` | `minimal-valid` | 1 | 32004 | 32004 | N/A | same-call local-only perf probe |
| `audit_scaling_probe` | `audit_plan_create_worker_probe` | `empty-pool` | 1 | 64829 | 64829 | N/A | same-call local-only perf probe |
| `root` | `canic_request_delegation` | `fresh-shard` | 1 | 1693922 | 1693922 | N/A |  |
| `test` | `test` | `minimal-valid` | 1 | 819 | 819 | N/A |  |
| `root` | `canic_response_capability_v1` | `cycles-request` | 1 | 490533 | 490533 | N/A |  |
| `root` | `canic_template_stage_manifest_admin` | `single-chunk` | 1 | 415038 | 415038 | N/A |  |
| `root` | `canic_template_prepare_admin` | `single-chunk` | 1 | 191031 | 191031 | N/A |  |
| `root` | `canic_template_publish_chunk_admin` | `single-chunk` | 1 | 337971 | 337971 | N/A |  |
...
| --- | --- | --- | ---: | ---: | ---: |
| `root:canic_request_delegation:fresh-shard` | `canic_core::ops::auth::delegation` | `sign_cert` | 1 | 338409 | 338409 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `encode_install_request` | 1 | 296650 | 296650 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `decode_response` | 1 | 214588 | 214588 |
| `root:canic_template_publish_chunk_admin:single-chunk` | `canic_control_plane::storage::stable::template::chunked` | `chunk_store_insert` | 1 | 138705 | 138705 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::replay` | `prepare_replay_input` | 1 | 137605 | 137605 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::execute` | `cache_root_verifier_proof` | 1 | 102238 | 102238 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `prepare_call` | 1 | 69896 | 69896 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler::replay` | `evaluate_replay` | 1 | 64614 | 64614 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::ops::auth::delegation` | `hash_cert` | 1 | 64147 | 64147 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `execute_call` | 1 | 61038 | 61038 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::auth` | `push_verifiers` | 1 | 58192 | 58192 |
| `root:canic_request_delegation:fresh-shard` | `canic_core::workflow::rpc::request::handler` | `commit_replay` | 1 | 50980 | 50980 |
...
| --- | --- | ---: | --- | --- |
| 1 | `root:canic_request_delegation:fresh-shard` | 1693922 | Shared runtime surface | [endpoints](/home/adam/projects/canic/crates/canic/src/macros/endpoints.rs) |
| 2 | `root:canic_response_capability_v1:request-cycles-fresh` | 490533 | Root dispatcher plus replay/capability workflow | [request handler](/home/adam/projects/canic/crates/canic-core/src/workflow/rpc/request/handler/mod.rs), [replay workflow](/home/adam/projects/canic/crates/canic-core/src/workflow/rpc/request/handler/replay.rs) |
| 3 | `root:canic_template_stage_manifest_admin:single-chunk` | 415038 | Shared runtime surface | [endpoints](/home/adam/projects/canic/crates/canic/src/macros/endpoints.rs) |
...
| Highest sampled endpoint currently highest-cost | WARN | `root:canic_request_delegation:fresh-shard` averages 1693922 local instructions in this first baseline. |
...
| `canic_metrics(MetricsKind::Perf, PageRequest { limit=512, offset=0 })` | PASS | Update scenarios were sampled before/after through persisted perf rows, and query scenarios used same-call local-only probe endpoints because query-side perf rows are not committed; normalized rows saved under `/home/adam/projects/canic/docs/audits/reports/2026-04/2026-04-06/artifacts/instruction-footprint-14/perf-rows.json`. |
| `repo checkpoint scan` | PASS | Found 69 checkpoint call sites. |
| `checkpoint delta capture` | PASS | 25 non-zero checkpoint delta rows were captured under `/home/adam/projects/canic/docs/audits/reports/2026-04/2026-04-06/artifacts/instruction-footprint-14/checkpoint-deltas.json`. |
...
- [instruction-footprint-14.md](./instruction-footprint-14.md)
- [scenario-manifest.json](artifacts/instruction-footprint-14/scenario-manifest.json)
- [perf-rows.json](artifacts/instruction-footprint-14/perf-rows.json)
- [endpoint-matrix.tsv](artifacts/instruction-footprint-14/endpoint-matrix.tsv)
- [checkpoint-deltas.json](artifacts/instruction-footprint-14/checkpoint-deltas.json)
- [flow-checkpoints.log](artifacts/instruction-footprint-14/flow-checkpoints.log)
- [checkpoint-coverage-gaps.json](artifacts/instruction-footprint-14/checkpoint-coverage-gaps.json)
- [verification-readout.md](artifacts/instruction-footprint-14/verification-readout.md)
- [method.json](artifacts/instruction-footprint-14/method.json)
- [environment.json](artifacts/instruction-footprint-14/environment.json)
