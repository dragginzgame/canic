...
if [[ ! -f $fixture || $(head -n 1 "$fixture") != "$expected_header" ]]; then
  echo "invalid change-friction v3 sample fixture" >&2
  exit 3
fi
...
if [[ $current_scope_files -eq 0 ]]; then
  echo "change-friction v3 current scope is empty" >&2
  exit 3
fi
...
if [[ $expected_order -ne 6 ]]; then
  echo "change-friction v3 requires exactly five frozen sample rows" >&2
  exit 3
fi

echo -e "identity\tvalue"
echo -e "method\tCANIC-CHANGE-FRICTION-001/v3"
  ...
  awk -v order="$order" -v commit="$commit" -v type="$slice_type" \
    ...
    'BEGIN {
      printf "%d\t%s\t%s\t%s\t%d\t%d\t%d\t%d\t%d\t%.3f\t%.3f\t%.3f\t%.3f\n",
        ...
        primary_module_files / files, subsystems / 24
    }' | tee -a "$slice_metrics"
done
