...
classify_file() {
  ...
  case ${relative%%/*} in
    access | api | bootstrap | cdk | config | diagnostics | dispatch | domain | dto | format | ids | infra | ingress | lifecycle | memory | model | ops | storage | view | workflow)
      printf '%s\tproduction\n' "${relative%%/*}"
      ;;
    replay_policy)
...
echo -e "method\tCANIC-COMPLEXITY-001/v3"
