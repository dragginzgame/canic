...
- [B5-B7 implementation progress](b5-b7-progress.md) records the resolved
  published-upstream B5 result and the reproducible B6-B7 evidence.
...
| CANIC-013 | At B1 capture, `ic-query 0.40.1` lost known Registry-version and cache-attempt context and had no `Unknown(reason)` retry result. The smallest additive upstream result was frozen without a fork or string parsing. | Resolved in B5 by published `ic-query 0.41.0` and exact typed host/CLI propagation. |
