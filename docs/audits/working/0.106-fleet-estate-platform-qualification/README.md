# Canic 0.106 B1 Working Evidence
...
[0.106 design](../../../design/0.106-fleet-estate-platform-qualification/0.106-design.md).
It is accepted local evidence, not a B2 qualification report. B2 disposable or
IC-mainnet effects remain unauthorized.

The immutable direct predecessor is annotated tag `v0.105.0`, peeled commit
`b6c46ca1d307e0a3fed6f7bfddfba7d9f1922811`. The accepted 0.104 timer and
recovery boundary remains inherited unchanged in the pool paths inventoried
here.

## Contents

- [B1 baseline and current inventory](b1-baseline-inventory.md) freezes source
  and tool identity, records the exact empty-topology blocker, and inventories
  the current root-owned pool state, encoded sizes, receipt/cost ceilings,
  scans, creation lane, snapshot ownership and native timers.
- [Cycles Ledger provenance and protocol](b1-protocol.md) freezes the exact
  deployed v1.0.6 interface/source authority, separates contract facts from
  pending observations and records the full-request retry requirements that
  the B2 harness must preserve.
...
- Cycles transferred or consumed by qualification: zero.
- Controllers changed: zero.
