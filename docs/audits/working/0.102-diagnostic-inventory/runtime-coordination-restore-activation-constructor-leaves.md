...
| --- | ---: | --- | --- | --- |
| reuse `ACCESS_ROOT_REQUIRED` | 1 | `Invariant` wrapper around exact root access | source identity | Start root-only services only on the protected Fleet Subnet Root |
| transparent typed memory-bootstrap cause | 1 | post-upgrade memory initialization | source projection | Preserve the exact `ic-memory` adapter diagnostic |
| `ICP_REFILL_UPGRADE_RESUMABLE` | 1 | `Invariant` / root upgrade admission | self | Complete or explicitly recover every resumable refill operation before reinstalling |
...
| Exact candidate or disposition | Sites | Class/origin | Public projection | Action and retry |
| --- | ---: | --- | --- | --- |
| reuse `AUTHORITY_RESTORE_UPDATE_FENCED` | 1 | `Conflict` / sealed update policy | self | Resume the exact sealed operation before invoking ordinary updates |
| `AUTHORITY_RESTORE_TIMER_RUNNING` | 1 | `Invariant` / snapshot interruption boundary | self | Wait for the running Canic timer to become resumable before sealing or resuming |
| `AUTHORITY_RESTORE_RUNTIME_REQUIRED` | 1 | `Forbidden` / authority runtime | self | Invoke snapshot fencing only on the Fleet Coordinator or Fleet Subnet Root |
...
| --- | ---: | --- | --- |
| reuse `FLEET_ACTIVATION_IDENTITY_MISMATCH` | 1 | resume request differs from protected root operation/credential | Preserve exact operation identity |
| reuse `FLEET_ACTIVATION_STATE_INVALID` | 3 | protected cascade manifest, root cascade or active credential is missing | Fail closed against the protected record |
| transparent typed activation-storage cause | 2 | non-root activation mutation and endpoint-status lookup | Preserve exact record/transition diagnostic |
| reuse `FLEET_ACTIVATION_ENDPOINT_FENCED` | 1 | Prepared endpoint is not admitted | Preserve exact phase-policy denial |
| split reuse of `FLEET_ACTIVATION_IDENTITY_MISMATCH`, `FLEET_ACTIVATION_EVIDENCE_MISMATCH`, `FLEET_ACTIVATION_STATE_INVALID` and `FLEET_ACTIVATION_TRANSITION_INVALID` | 1 | one combined child-status predicate | Split identity, cascade, forbidden root-only evidence, timestamp and required-phase predicates before allocation |
| `FLEET_ACTIVATION_CREDENTIAL_BUNDLE_REQUIRED` | 1 | `Invariant` / unsupported prepared credential authority | self | Use the credential-bundle activation slice before configuring issuer policy/templates |
| reuse `FLEET_ACTIVATION_WASM_STORE_PRINCIPAL_INVALID` | 2 | Store principal is anonymous or equals the root | self | Supply one distinct non-anonymous sibling Store principal |
