...
handled in the next section. Eighteen other operation variants have production
constructors. Four delegated-token time identities in the table are instead
owned by the maintained delegated-token errors: their same-named
`AuthExpiryError` variants have no production constructor and are B4 sediment.
...
| `AUTH_CERT_EXPIRED` | delegated-token preparation, verification and active-proof installation `CertExpired` | `Unauthorized` / delegation certificate | self | Renew certificate; retry with new proof |
| `AUTH_TOKEN_EXPIRED` | `VerifyDelegatedTokenError::TokenExpired` | `Unauthorized` / delegated token | self | Renew token; retry with new token |
| `AUTH_TOKEN_NOT_YET_VALID` | `VerifyDelegatedTokenError::TokenNotYetValid` | `Unauthorized` / delegated token | self | Wait until issue time or correct clock/evidence |
| `AUTH_TOKEN_TTL_EXCEEDED` | delegated-token preparation and verification `TokenTtlExceeded` | `Forbidden` / delegated token policy | self | Request token within maximum TTL |
...
The unproduced `AuthExpiryError::{CertExpired, TokenExpired,
TokenNotYetValid, TokenTtlExceeded}` variants add no identity and must be
deleted in B4. Their labels remain required because the producer-reachable
delegated-token enums above own those exact current meanings. A full-workspace
source search finds no constructor for any of the four dead variants.
