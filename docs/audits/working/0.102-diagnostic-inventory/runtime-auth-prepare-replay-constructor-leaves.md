...
| --- | ---: | --- | --- | --- |
| `AUTH_PREPARE_REPLAY_EXPIRY_OVERFLOW` | 1 | `InvalidInput` / replay expiry arithmetic | self | Correct request time or replay TTL before reserving |
| `AUTH_PREPARE_OPERATION_ID_REQUIRED` | 2 | `InvalidInput` / replay identity | self | Supply one exact nonzero request identity |
| `AUTH_ROLE_ATTESTATION_REPLAY_TTL_ZERO` | 1 | `InvalidInput` / role-attestation replay metadata | self | Supply a positive replay TTL |
| `AUTH_ROLE_ATTESTATION_REPLAY_TTL_EXCEEDED` | 1 | `InvalidInput` / role-attestation replay metadata | self | Reduce replay TTL to the maintained ceiling |
| `AUTH_TOKEN_PREPARE_REPLAY_TTL_ZERO` | 1 | `InvalidInput` / delegated-token replay metadata | self | Supply a positive replay TTL |
| `AUTH_TOKEN_PREPARE_REPLAY_TTL_EXCEEDED` | 1 | `InvalidInput` / delegated-token replay metadata | self | Reduce replay TTL to the maintained ceiling |
...
| Exact candidate | Sites | Class/origin | Public projection | Action and retry |
| --- | ---: | --- | --- | --- |
| `REPLAY_UNEXPECTED_FRESH_DECISION` | 2 | `Invariant` / workflow dispatch | self | Repair the reserve/dispatch boundary; no unchanged retry |
| `REPLAY_OPERATION_IN_PROGRESS` | 2 | `Conflict` / exact replay operation | self | Retry later with the same request ID and payload |
| `REPLAY_ACTOR_MISMATCH` | 2 | `Conflict` / replay actor identity | self | Never reuse another actor's request ID |
| `REPLAY_PAYLOAD_MISMATCH` | 2 | `Conflict` / replay payload identity | self | Replay only the exact original payload or use a new request ID |
| `REPLAY_RECEIPT_EXPIRED` | 2 | `Conflict` / replay retention | self | Start a new operation with a new request ID |
| `AUTH_PREPARE_REPLAY_RECOVERY_REASON_INVALID` | 2 | `Conflict` / auth response recovery | self | Inspect the exact retained receipt; auth preparation may auto-recover only response-commit failure |
| `REPLAY_PENDING_ACTOR_CAPACITY` | 2 | `ResourceExhausted` / per-actor pending receipts | self | Wait for exact pending operations to settle before retry |
| `REPLAY_PENDING_COMMAND_CAPACITY` | 2 | `ResourceExhausted` / per-command pending receipts | self | Wait for command-kind capacity before retry |
...
| Exact candidate | Sites | Class/origin | Public projection | Action and retry |
| --- | ---: | --- | --- | --- |
| `REPLAY_RECEIPT_MISSING` | 1 | `Workflow` / retained replay state | self | Preserve the operation and inspect missing durable receipt state |
| `REPLAY_RECEIPT_DECODE_FAILED` | 1 | `Workflow` / retained replay encoding | self | Preserve malformed bytes and fail closed |
| `REPLAY_RECEIPT_TOKEN_MISMATCH` | 1 | `Workflow` / protected receipt identity | self | Reload the exact receipt token; never commit through stale identity |
| `REPLAY_STAGED_RESPONSE_MISSING` | 1 | `Workflow` / response-commit recovery | self | Preserve receipt state and reconstruct only from exact staged bytes |
| `REPLAY_COST_GUARD_SETTLEMENT_MISSING` | 1 | `Workflow` / cost settlement identity | self | Preserve receipt state and fail before settlement or response commit |
| `REPLAY_RESPONSE_ENCODE_FAILED` | 2 | `Workflow` / canonical response encoding | self | Repair the exact response contract; no unchanged retry |
| `REPLAY_RESPONSE_DECODE_FAILED` | 2 | `Workflow` / terminal response reconstruction | self | Preserve terminal bytes and repair the exact response decoder |
