...
| Exact candidate | Typed source | Public projection | Action and retry |
| --- | --- | --- | --- |
| `WASM_STORE_STORED_CHUNKS_REQUEST_ENCODE_FAILED` | request Candid encoder | `IC_PLATFORM_PROTOCOL_INVALID` | Fix the maintained management DTO; unchanged runtime retry cannot help |
| `WASM_STORE_STORED_CHUNKS_LIQUID_CYCLES_INSUFFICIENT` | insufficient liquid cycles | self | Top up the root before retrying the exact observation |
| `WASM_STORE_STORED_CHUNKS_CALL_PERFORM_FAILED` | local call performance failure | `IC_PLATFORM_EFFECT_FAILED` | Retry only through the bounded publication attempt |
| `WASM_STORE_STORED_CHUNKS_REJECTED_SYSTEM_FATAL` | `SysFatal` | `IC_PLATFORM_EFFECT_FAILED` | Stop blind retry and inspect platform state |
| `WASM_STORE_STORED_CHUNKS_REJECTED_SYSTEM_TRANSIENT` | `SysTransient` | self | Bounded retry through the same publication attempt |
| `WASM_STORE_STORED_CHUNKS_REJECTED_DESTINATION_INVALID` | `DestinationInvalid` | self | Re-observe the exact adopted Store; this read rejection is not physical-deletion evidence |
| `WASM_STORE_STORED_CHUNKS_REJECTED_BY_CANISTER` | `CanisterReject` | `IC_PLATFORM_EFFECT_FAILED` | Inspect the maintained management/Store contract without exposing reject prose |
| `WASM_STORE_STORED_CHUNKS_REJECTED_CANISTER_ERROR` | `CanisterError` | `IC_PLATFORM_EFFECT_FAILED` | Inspect target health and retry only through the owning attempt |
| `WASM_STORE_STORED_CHUNKS_REJECTED_SYSTEM_UNKNOWN` | `SysUnknown` | `IC_PLATFORM_EFFECT_FAILED` | Preserve the attempt and reconcile outcome before retry |
| `WASM_STORE_STORED_CHUNKS_REJECT_CODE_UNKNOWN` | unrecognized raw reject code | `IC_PLATFORM_EFFECT_FAILED` | Fail closed and review the pinned CDK/IC boundary |
| `WASM_STORE_STORED_CHUNKS_RESPONSE_DECODE_FAILED` | response Candid decoder | `IC_PLATFORM_PROTOCOL_INVALID` | Preserve response context and repair the qualified interface |
...
| Exact candidate | Typed source | Public projection | Action and retry |
| --- | --- | --- | --- |
| `WASM_STORE_UPLOAD_CHUNK_REQUEST_ENCODE_FAILED` | request Candid encoder | `IC_PLATFORM_PROTOCOL_INVALID` | Fix the maintained management DTO before retry |
| `WASM_STORE_UPLOAD_CHUNK_LIQUID_CYCLES_INSUFFICIENT` | insufficient liquid cycles | self | Top up the root and exact-retry the same protected chunk attempt |
| `WASM_STORE_UPLOAD_CHUNK_CALL_PERFORM_FAILED` | local call performance failure | `IC_PLATFORM_EFFECT_FAILED` | Reconcile the retained attempt before retrying the effect |
| `WASM_STORE_UPLOAD_CHUNK_REJECTED_SYSTEM_FATAL` | `SysFatal` | `IC_PLATFORM_EFFECT_FAILED` | Stop blind retry and inspect platform state |
| `WASM_STORE_UPLOAD_CHUNK_REJECTED_SYSTEM_TRANSIENT` | `SysTransient` | self | Bounded exact retry through the same attempt |
| `WASM_STORE_UPLOAD_CHUNK_REJECTED_DESTINATION_INVALID` | `DestinationInvalid` | self | Re-observe the exact target Store; never infer successful upload |
| `WASM_STORE_UPLOAD_CHUNK_REJECTED_BY_CANISTER` | `CanisterReject` | `IC_PLATFORM_EFFECT_FAILED` | Inspect the typed Store/management contract; discard reject prose |
| `WASM_STORE_UPLOAD_CHUNK_REJECTED_CANISTER_ERROR` | `CanisterError` | `IC_PLATFORM_EFFECT_FAILED` | Preserve attempt identity and inspect target health |
| `WASM_STORE_UPLOAD_CHUNK_REJECTED_SYSTEM_UNKNOWN` | `SysUnknown` | `IC_PLATFORM_EFFECT_FAILED` | Treat the effect outcome as unresolved until exact reconciliation |
| `WASM_STORE_UPLOAD_CHUNK_REJECT_CODE_UNKNOWN` | unrecognized raw reject code | `IC_PLATFORM_EFFECT_FAILED` | Fail closed and review the pinned adapter surface |
| `WASM_STORE_UPLOAD_CHUNK_RESPONSE_DECODE_FAILED` | response Candid decoder | `IC_PLATFORM_PROTOCOL_INVALID` | Preserve the attempt and repair the response contract |
...
The qualified semantic set moves from 2,822 to 2,844 exact candidates. The 31
safe projections remain unchanged, producing 2,875 current symbolic identities.
