...
| `COMPONENT_STORE_ARTIFACT_UNAVAILABLE` | 1 | Verified Store catalog lacks the admitted top-level or child role artifact | self | Publish/repair the exact root release set before retry | public |
| `COMPONENT_STORE_ARTIFACT_DUPLICATE` | 1 | Verified Store catalog contains duplicate entries for one top-level or child role | `COMPONENT_REGISTRY_STATE_INVALID` | Fail closed and repair/reinstall catalog authority | recent failure |
