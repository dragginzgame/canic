...
| --- | ---: | --- | --- |
| transparent typed Fleet-activation/storage cause | 1 | protected Fleet lookup | Preserve the exact storage/activation diagnostic without a facade wrapper |
| transparent typed `AuthPolicyError` dispatch | 1 | issuer-policy upsert | Exhaustively map each producer-reachable policy variant to its qualified identity |
| reuse `AUTH_ROOT_ISSUER_CERT_TTL_ZERO` | 1 | renewal-template TTL admission | Return the existing exact request-policy diagnostic |
| transparent typed `AuthPolicyError` dispatch | 2 | renewal-template Fleet/grant/policy admission | Preserve each exact policy identity, class and approved projection |
...
| Exact candidate or disposition | Sites | Current boundary | Required hard cut |
| --- | ---: | --- | --- |
| transparent typed `AuthPolicyError` dispatch | 1 | per-issuer batch approval | Preserve the exact policy diagnostic and protected issuer/batch correlation |
