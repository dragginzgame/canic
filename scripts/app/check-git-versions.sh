...
# Check available git versions for ICU integration
set -e
...
REPO_URL="https://github.com/dragginzgame/icu"

echo "🔍 Checking available ICU versions for git dependencies..."
echo ""
...
for tag in $latest_tags; do
    ...
    echo '```toml'
    echo "[dependencies]"
    echo "icu = { git = \"$REPO_URL\", tag = \"$tag\", features = [] }"
    echo '```'
    echo ""
done
...
echo '```toml'
echo "[dependencies]"
echo "icu = { git = \"$REPO_URL\", branch = \"main\", features = [] }"
echo '```'
echo ""

print_info "Feature flags you can use:"
echo "  - default (always enabled)"
echo "  - auth (authentication support)"
echo "  - interface (interface utilities)"
...
echo '```toml'
echo "[dependencies]"
echo "icu = { git = \"$REPO_URL\", tag = \"v0.1.12\", features = [\"auth\", \"interface\"] }"
echo '```'
echo ""

print_info "To check if a specific version exists:"
echo "  git ls-remote --tags $REPO_URL | grep v0.1.12"
echo ""
