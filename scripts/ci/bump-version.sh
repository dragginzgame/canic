...
# Own the visible snapshot as well as its marker. Preserve human prose verbatim
# as the pre-transaction handoff instead of treating its versions as release facts.
awk '
  /^<!-- canic-status-summary:start -->$/ && !summary {
    summary = 1; retained = $0 ORS; next
  }
  /^<!-- canic-status-summary:end -->$/ && summary {
    summary = 0; retained = ""; next
  }
  summary { retained = retained $0 ORS; next }
  { print }
  END { printf "%s", retained }
' "$STATUS_DOCUMENT" >"$TRANSACTION_DIR/status-handoff.md"
{
  printf '%s\n' '<!-- canic-status-summary:start -->' '# Validated release snapshot' ''
  printf -- '- Package version: `%s`\n- Release date: `%s`\n' "$NEW" "$RELEASE_DATE"
  printf -- '- Validated source: `%s`\n- Validation gate: `%s`\n' "$CURRENT_HEAD" "$VALIDATION_KIND"
  printf '\n%s\n' \
  ...
  cat "$TRANSACTION_DIR/status-handoff.md"
  printf '\n%s\n' "$VALIDATION_STATUS_MARKER"
} >"$STATUS_DOCUMENT"
