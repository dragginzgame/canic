...
- `0.18.5` keeps `ICRC-21` behind role-scoped compile-time gating and trims the shared generated surface by making `canic_app_state` and `canic_subnet_state` root-only, with integration coverage that asserts the endpoints stay present on `root` and absent on ordinary child canisters.
