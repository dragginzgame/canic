...
- `0.18.5` keeps `ICRC-21` behind role-scoped compile-time gating, trims the shared generated surface by making `canic_app_state` and `canic_subnet_state` root-only, and hardens bundle builds so `root` and `wasm_store` stop reusing stale `.dfx/local` artifacts from the wrong profile when embedding dependent wasm payloads.
