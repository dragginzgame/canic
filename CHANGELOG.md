...
- `0.24.3` folds sharding back into `canic-core`, removes the standalone `canic-sharding-runtime` crate and the extra `xxhash-rust` dependency, keeps the `canic` `sharding` feature stable for facade users, switches HRW scoring to `sha2`, and tightens the internal root test surface around explicit topology, scaling, and sharding harness profiles so hierarchy, replay, delegation, and instruction-audit suites stop dragging unrelated canisters into setup.
