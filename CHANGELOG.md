...
- `0.22.8` cleans up the repo-local/downstream output so build wrappers print the workspace/DFX roots once per run, show the selected `debug|fast|release` build profile, log per-canister elapsed time with `0.01s` precision, render the installer’s end-of-run timing summary as a readable table, and make local `canic-install-root` top up root cycles only when the current balance is below the local install floor.
