...
- `0.24.3` folds sharding back into `canic-core`, removes the standalone `canic-sharding-runtime` crate and the extra `xxhash-rust` dependency, keeps the `canic` `sharding` feature stable for facade users, switches HRW scoring to `sha2`, keeps the root hierarchy focused on the cases that really need it by moving standalone `app`, `test`, and `scale_hub` checks onto a shared internal PocketIC fixture, moves the heavy internal `canic_env` / memory-registry / directory queries behind a test-only build flag, and makes the local tooling less fragile by auto-recovering the local `dfx` install flow once and letting the wasm audit build artifacts without depending on a healthy replica first.
