...
- `0.26.5` finishes the delegated-auth verifier bootstrap fix by having root push the delegation public key alongside the proof itself, so verifier-only canisters can install and verify delegation material without needing their own `auth-crypto` threshold-ECDSA support.
- `0.26.4` keeps the late `0.26` follow-through on the clean side by splitting more `canic-testkit` and runtime ownership seams, making `wasm_store.did` refresh explicit instead of incidental during normal bootstrap builds, and fixing the workspace test runner so the PocketIC suites follow their moved `canic-tests` package targets.
