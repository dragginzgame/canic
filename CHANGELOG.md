...
- `0.21.1` hardens the first managed-fleet release by scoping and pruning stale approved roles to the current config-driven release set, keeping the implicit `wasm_store` preset downstream-safe without const-only assumptions, and removing the local `dfx` smoke path from `make test` / `make test-bump` so the normal test gate stays PocketIC/Cargo-driven while manual `dfx` installs still fail fast if the replica is not already running.
