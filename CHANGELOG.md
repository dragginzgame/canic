...
- `0.110.15` improves Fleet planning, selected-build reinstall and recovery,
  ...
  publication/recovery, corrected importer layering, breaking review records,
  public memory summaries, final Wasm code/data reporting and IcyDB 0.257.5 fixture compatibility.
...
- `0.110.5` fixes terminal Fleet inventory validation across source-to-
  ...
  bootstrap from an exact runtime replay, lets a still-runnable draining
  Component complete its final Directory convergence before quiescence, makes
  reports readable and bounded, and adds public managed Component-tree
  qualification plus a literal-zero production-adapter Fleet Ensure proof.
...
- `0.109.33` completes the host-only managed-App qualification package boundary,
  ...
  and hard-cuts Root and Coordinator commands to explicitly named role
  entrypoints.
...
- `0.109.29` corrects `CANIC-098` by crossing one exact rejected predecessor
  ...
  predecessor module identities without defaulting them. The fresh plan
  reinstalls Store then Root, performs successor-only adoption, conserves
  controlled cycles and immediately replays without effect.
...
- `0.109.5` rejects undersized initial pool policy before effects while loading
  ...
  lets the protected Root pool owner re-inspect a topped-up imported asset
  without replacing its retained provisioning operation. Recovery diagnostics
  for this policy mismatch no longer recommend irrelevant identity or Ledger
  funding changes.
...
- `0.107.2` adds digest-stable live catalog acquisition for cold-cache plans
...
- `0.104.1` strengthens the timer-ownership contract from lexical hits to
  ...
  freezes the exact combined Canic/IcyDB timer inventories, and replaces
  unsupported phase deltas with reproducible exact-release footprint evidence
  while aligning the complete closeout record.
...
- `0.103.0` closes the 0.102 and 0.103 audits with profile-exact role control
  planes, pre-dispatch authorization and immutable caller bindings, reducing
  188 representative Canic method appearances to ten. It also removes the
...
- `0.101.46` lets an exact tagged release commit push while later local edits
  ...
  committed `HEAD`, and records the provisional 0.110 Skynet design reservoir.
...
- `0.101.26` advances only the durably reserved scale-out batches through the
  existing response-loss-safe root-acceptance protocol, retaining historical
  terminal receipts while keeping provisioning and publication fenced.
...
- `0.101.23` persists explicit initial placement-to-Subnet authority and drives
  ...
  and makes structural cleanup plus sediment removal a mandatory 0.101
  closeout gate.
...
- `0.101.2` adds bounded independent group deployments with exact path-resolved
  ...
  content-addressed test-Wasm reuse.
...
- `0.100.103` replaces unrecoverable raw Canister creation with exact-Subnet
  Cycles Ledger pool refill, removes aggregate physical caps, unifies physical
  inventory, and advances the host-only Subnet Catalog to `ic-query 0.28.4`.
...
- `0.100.101` relays an operator-owned password file to every ICP subprocess,
  ...
  Subnet Catalog boundary to `ic-query 0.26.15` with its NNS-scoped cache hard
  cut.
...
- `0.100.100` enforces the durable root-deletion preparation receipt as a
  ...
  effect-free exact terminal replay. It also refreshes the proposed 0.101
  design against current host-installed Store, prepaid-Canister and root-limit
  authority, closes its cross-root requester-proof handoff and records a
  measured initial scale envelope instead of claiming ten-thousand-Subnet
  qualification.
...
- `0.100.84` routes structural root provisioning through the resumable
  ...
  into its local child cache. It also advances host-only `ic-query` to
  `0.25.3` through its focused Subnet Catalog feature.
...
- `0.100.68` consolidates duplicated Fleet-install ICP context construction
  under one host boundary, removing redundant helpers without changing command,
  authority or public contract behavior.
...
- `0.100.66` atomically converges mixed `Active`/`Draining` Fleet Registry
  ...
  abandoned DTOs and redundant host/runtime surfaces while retaining current
...
- `0.100.65` atomically publishes one locally fenced Fleet Subnet Root as
  ...
  lifecycle-history validation, and adopts the patched transitive
  `event-listener 5.4.2` release.
...
- `0.100.38` advances exact direct-child reservations through Store-bound,
  cost-guarded root creation with durable intent, sole-root control and
  interruption-safe principal evidence.
...
- `0.100.34` completes durable empty-inventory Fleet Subnet Root activation
  with uncertain-call reconciliation and independent terminal runtime and
...
- `0.100.11` adds the genuine built-in Fleet Coordinator runtime and its
  ...
  before release-build finalization.
...
- `0.100.3` makes exact infrastructure artifact evidence immutably durable
  under its release-build identity with fail-closed complete-build admission
  and same-release retry.
...
- `0.99.29` hard-cuts runtime inspection and Medic targeting from
  ...
  guidance, and removes invalid same-name conflation warnings. Its design
  amendment keeps the Fleet Coordinator outside every Canister Tree and
  publishes Tree-owned singleton services without transferring their local
  lifecycle authority.
...
- `0.99.27` completes fresh Fleet installation: the host atomically publishes
  ...
  only while the catalog row and root-install receipt remain valid. Its
  follow-on design update replaces Subnet-slot membership with explicit Tree
  Specs, Tree Groups and co-locatable Canister Trees.
...
- `0.99.14` gives every managed non-root one exact Fleet-bound fresh-install
  ...
  local lifecycle without fabricated Fleet identity.
...
- `0.98.21` removes the unsupported ICRC-103 configuration and advertisement
  path, and restores one canonical instrumented `canic::api::call::Call`
  builder over Canic's surviving inter-canister transport.
...
- `0.98.12` removes unused delegated-auth call-kind forwarding, internal
  ...
  warning array. Finding-local warning evidence remains intact.
...
- `0.95.5` replaces periodic full-history log retention with append-owned
  count bounds and one exact, bounded age-deadline owner that remains idle
  when no age policy is configured.
...
- `0.93.27` is potentially breaking: supplied deployment plans now normalize
  ...
  release publication, and activation consume only that verified authority.
...
- `0.93.25` is potentially breaking: selected ICP targets now use the
  ...
  networks retain their distinct network names.
...
- `0.93.0` is potentially breaking: it hard-cuts stale pre-1.0 APIs,
  serialized states, and operator projections so maintained contracts expose
  only production-backed behavior.
...
- `0.92.8` is potentially breaking: it makes CI, maintainer-tool, and release
  ...
  Ubuntu/Linux/Wasm support authority.
...
- `0.86.8` completes state-manifest decomposition with focused audit and
  aggregation owners while preserving finding, report, serialization, and
...
- `0.85.5` rejects invalid static release artifact shapes before staging and
  adopts `ic-query 0.10` without changing Canic's subnet-catalog integration.
...
- `0.85.0` makes restore recovery persistence durable, replaces global build
  ...
  Canic-owned CBOR to `ciborium` with exact stable and wire-byte proof.
...
- `0.84.6` makes placement-capacity and sharding state declarations truthful
  with canonical `*Data` snapshots and owner-defined descriptor names while
  preserving stable-memory IDs and encodings.
...
- `0.83.24` tightens deployment-truth status label ownership by moving safety,
  ...
  the deployment-truth model enums while preserving operator text output
  labels, medic summaries, command behavior, JSON schemas, and deployment truth
  schema.
...
- `0.83.18` tightens deployment-truth comparison report validation and text
  ...
  with typed labels while preserving validation error field strings, operator
  text output labels, command behavior, JSON schemas, deployment truth schema,
...
- `0.83.15` tightens host install-root deployment-truth receipt phase
  ...
  receipt phase strings, operation IDs, timing table labels, command behavior,
  JSON schemas, deployment truth schema, and stable-state layout.

- `0.83.14` tightens runtime bootstrap status phase ownership by replacing
  raw internal root/nonroot bootstrap phase labels with `BootstrapPhaseLabel`
  while preserving `canic_bootstrap_status` phase strings, runtime
  introspection metadata, endpoint surfaces, Candid, JSON, deployment truth,
  evidence/report schemas, and stable-state layout.
...
- `0.83.13` tightens replay-policy manifest construction by requiring typed
  ...
  preserving runtime replay `CommandKind` handling, endpoint surfaces,
  command behavior, Candid, JSON, and stable-state layout.
...
- `0.83.2` tightens `canic inspect` report ownership by replacing raw-string
  ...
  values while preserving existing JSON labels, text output meaning, command
  behavior, endpoint surfaces, Candid, and stable-state layout.
...
- `0.82.39` hard-cuts the public ICRC-21 dispatch handler alias and public
  ...
  behavior, and serialized protocol surfaces.
...
- `0.82.29` narrows workflow prelude usage across pool and IC workflow
  ...
  stale `workflow::prelude::Account` re-export.
...
- `0.82.6` continues app-state DTO/storage boundary hardening by moving app
  ...
  Candid shape, and stable app-state serialization.
...
- `0.82.3` continues DTO/domain boundary hardening by moving internal value
  ...
  and serialized Candid shapes.

- `0.82.2` hardens release bump safety and DTO ownership boundaries by
  confirming minor/major bumps before release gates and moving internal
  runtime value semantics into domain ownership while preserving public DTO
  compatibility.
