...
- `audit/` – internal audit and performance probe canisters used by instruction,
  wasm-size, and capability-surface audits, including separate unconfigured
  `minimal` and configured `minimal_metrics` shared runtime baselines.
...
- Build audit probes through Cargo; `canister_minimal` is the unconfigured floor
  and `canister_minimal_metrics` configures the leaf metrics profile:
